...
def preproc_IFS(
    ...
    *******
    ...
    if local_path:
        f1 = join(vcal_path, "Static/sphere.json")
        f2 = join(vcal_path, "Static/SPHERE_CPI_ND.dat")
    else:
        try:
            f1 = download_file(join(url_d, "sphere.json"), cache=True)
            f2 = download_file(join(url_d, "SPHERE_CPI_ND.dat"), cache=True)
        except:
    ...
    with open(f1, "r") as instr_param_file:
    ...
    nd_filename = f2  # FILE WITH TRANSMISSION OF NEUTRAL DENSITY FILTER
    ...
    # parts of pipeline
    to_do = params_preproc.get(
        "to_do", {1, 2, 3, 4, 5, 6, 7, 8}
    )  # parts of pre-processing to be run.
    ...
    imlib = params_preproc.get(
        "imlib", "vip-fft"
    )  # image processing library to be used
    nproc = params_preproc.get(
    ...
    cen_box_sz = params_preproc.get(
        "cen_box_sz", [31, 21, 31]
    )  # size of the subimage for centering fits
    ...
    bp_crop_sz = params_preproc.get(
        "bp_crop_sz", 261
    )  # crop size before bad pix correction for OBJ, PSF and CEN
    final_crop_sz = params_preproc.get(
        "final_crop_sz", [257, 256]
    )  # 361 => 2.25'' radius; but better to keep it as large as possible and only crop before post-processing. Here we just cut the useless edges (100px on each side)
    final_crop_sz_psf = params_preproc.get(
        "final_crop_sz_psf", 17
    )  # 51 => 0.25'' radius (~3.5 FWHM)
    psf_model = params_preproc.get(
        "psf_model", "gauss"
    )  #'airy' #model to be used to measure FWHM and flux. Choice between {'gauss', 'moff', 'airy'}
    bin_fac = params_preproc.get(
        "bin_fac", 1
    ...
    n_med_sdi = params_preproc.get(
        "n_med_sdi", [1, 2, 3, 4, 5]
    )  # number of channels to combine at beginning and end
    ...
    OBJ_IFS_list = [
        x[:-5] for x in listdir(inpath) if x.startswith(prefix[0])
    ]  # don't include ".fits"
    OBJ_IFS_list.sort()
    ...
    PSF_IFS_list = [
        x[:-5] for x in listdir(inpath) if x.startswith(prefix[1])
    ]  # don't include ".fits"
    ...
    CEN_IFS_list = [
        x[:-5] for x in listdir(inpath) if x.startswith(prefix[2])
    ]  # don't include ".fits"
    ...
    if ncen > 0:
        ...
        labels.append("_cen")
        labels2.append("cen")
        final_crop_szs.append(
            final_crop_sz
        )  # add the same crop for CEN as for OBJ
    ...
    if bool(to_do):
        ...
        if not use_cen_only:
            cube, header = open_fits(
                inpath + OBJ_IFS_list[0] + ".fits", header=True, verbose=debug
            )
        else:  # case there are only CEN files
            cube, header = open_fits(
                inpath + CEN_IFS_list[0] + ".fits", header=True, verbose=debug
            )
        ...
        nd_file = read_csv(
            ...
            engine="python",
            header=None,
            names=["wavelength", "ND_0.0", "ND_1.0", "ND_2.0", "ND_3.5"],
        )
        ...
        if 1 in to_do:
            ...
            for file_list in obj_psf_list:
                for fi, filename in enumerate(file_list):
                    ...
                    if (
                        not isfile(
                            outpath + "{}_1bpcorr.fits".format(filename)
                        )
                        or overwrite[0]
                    ):
                        cube, header = open_fits(
                            inpath + filename, header=True, verbose=debug
                        )
                        mask = open_fits(
                            inpath + filename, n=1, verbose=debug
                        )  # mask from esorex showing locations of empty data
                        cube[mask == 2] = (
                            0  # set values of 2 (empty data) in the mask to 0 in the image
                        )
                        if cube.shape[1] % 2 == 0 and cube.shape[2] % 2 == 0:
                            cube = cube[:, 1:, 1:]
                        if 0 < bp_crop_sz < cube.shape[1]:
                            cube = cube_crop_frames(
                                cube, bp_crop_sz, verbose=debug
                            )
                        ...
                        cube = cube_fix_badpix_clump(
                            ...
                            verbose=full_output,
                            half_res_y=False,
                            max_nit=max_bpix_nit,
                            full_output=full_output,
                            nproc=nproc,
                        )
                        if full_output:
                            write_fits(
                                outpath + filename + "_1bpcorr_bpmap.fits",
                                cube[1],
                                header=header,
                                verbose=debug,
                            )
                            cube = cube[0]
                        write_fits(
                            outpath + filename + "_1bpcorr.fits",
                            cube,
                            header=header,
                            verbose=debug,
                        )
        ...
        if 2 in to_do:
            print("\n************* 2. RECENTERING *************\n", flush=True)
            for fi, file_list in enumerate(
                obj_psf_list
            ):  # OBJECT, then PSF (but not CEN)
                if fi == 0:
                    negative = coro
                    rec_met_tmp = rec_met
                    all_mjd = []
                ...
                ):  # CEN does not need to be centered and are no longer used, only keep to make master cube
                    if (
                        not isfile(outpath + f"{file_list[-1]}_2cen.fits")
                        or overwrite[1]
                    ):
                        for fn, filename in enumerate(file_list):
                            system(
                                f"cp {outpath}{filename}_1bpcorr.fits {outpath}{filename}_2cen.fits"
                            )
                    print("\nFinished centering\n", flush=True)
                    break
                if (
                    ...
                    if isinstance(rec_met, list):
                        ...
                        for ii in range(len(rec_met_tmp)):
                            # find coords of max
                            if fi == 1 or not coro:
                                frame_conv = frame_filter_lowpass(
                                    np.median(cube, axis=0),
                                    fwhm_size=int(1.2 * max_resel),
                                )
                                idx_max = np.argmax(frame_conv)
                                yx = np.unravel_index(
                                    idx_max, frame_conv.shape
                                )
                                xy = (yx[1], yx[0])
                            else:
                                xy = None
                            if "2dfit" in rec_met_tmp[ii]:
                                ...
                                try:
                                    ...
                                    y_shifts, x_shifts = np.zeros(
                                        cube.shape[0]
                                    ), np.zeros(cube.shape[0])
                                ...
                                if not use_cen_only:
                                    ...
                                    if true_ncen is None or true_ncen == 0:
                                        true_ncen = ncen
                                    elif true_ncen > ncen or true_ncen > 4:
                                        raise ValueError(
                                            "Code not compatible with true_ncen > ncen or true_ncen > 4"
                                        )
                                    elif true_ncen > 2:
                                        header_fin = open_header(
                                            inpath + OBJ_IFS_list[-1] + ".fits"
                                        )
                                        mjd_fin = float(header_fin["MJD-OBS"])
                                    elif true_ncen > 3:
                                        header_mid = open_header(
                                            inpath
                                            + OBJ_IFS_list[int(nobj / 2)]
                                            + ".fits"
                                        )
                                    ...
                                    for cc in range(true_ncen):
                                        ...
                                        y_shifts_cen[cc] = np.median(
                                            y_shifts_cen_tmp[np.where(cond)][
                                                :
                                            ],
                                            axis=0,
                                        )
                                        x_shifts_cen[cc] = np.median(
                                            x_shifts_cen_tmp[np.where(cond)][
                                                :
                                            ],
                                            axis=0,
                                        )
                                    ...
                                    for zz in range(n_z):
                                        y_shifts[zz] = np.interp(
                                            [mjd],
                                            unique_mjd_cen,
                                            y_shifts_cen[:, zz],
                                        )
                                        x_shifts[zz] = np.interp(
                                            [mjd],
                                            unique_mjd_cen,
                                            x_shifts_cen[:, zz],
                                        )
                        ...
                        print(
                            "Best centering method for {}: {}".format(
                    ...
                    if isinstance(
                        rec_met_tmp, str
                    ):  # perform only one centering method as requested by the user
                        final_y_shifts = []
                        final_x_shifts = []
                        final_y_shifts_std = []
                        final_x_shifts_std = []
                        for fn, filename in enumerate(file_list):
                            ...
                            if fi == 1 or not coro:
                                cube_med = np.median(
                                    cube, axis=0
                                )  # median combine all channels
                                # search in subframe that avoids empty data in the edges
                                y_max, x_max = peak_coordinates(
                                    cube_med,
                                    fwhm=1,
                                    approx_peak=frame_center(cube_med),
                                    search_box=85,
                                )
                                xy = (x_max, y_max)
                                if debug:
                                    print(
                                        f"Rough xy position of star: {xy}",
                                        flush=True,
                                    )
                            else:
                                xy = None
                            if "2dfit" in rec_met_tmp:
                                ...
                                try:
                                    cube_dft = cube_recenter_dft_upsampling(
                                        cube,
                                        center_fr1=(xy[1], xy[0]),
                                        negative=negative,
                                        fwhm=1.2 * max_resel,
                                    ...
                                    cube_tmp = np.zeros(
                                        ...
                                        dtype=np.float32,
                                    )  # needs to have three dimensions
                                    cube_tmp[0] = np.median(
                                        cube_dft, axis=0
                                    )  # median combine all channels
                                    _, y_shifts, x_shifts = (
                                        cube_recenter_2dfit(
                                    ...
                                    if debug:
                                        print(
                                            f"dft{int(rec_met_tmp[4:])} + 2dfit centering: xshift: {x_shifts[0]} px, "
                                            f"yshift: {y_shifts[0]} px for cube {filename}_1bpcorr.fits",
                                            flush=True,
                                        )
                                    cube = cube_shift(
                                        cube_dft,
                                ...
                                if fn == 0 and (
                                    ...
                                    loD = (
                                        ...
                                        * 3600
                                        / plsc
                                    )  # 1lambda/D in detector px, rad -> deg -> arcsec -> detector px
                                    ...
                                    for cc in range(ncen):
                                        ...
                                        for idx in range(len(lbdas)):
                                            for spot in range(4):
                                                spot_xy[idx][spot][0] = ref_xy[
                                                    0
                                                ] + factor * loD[idx] * np.cos(
                                                    orient + np.pi / 2 * spot
                                                )
                                                spot_xy[idx][spot][1] = ref_xy[
                                                    1
                                                ] + factor * loD[idx] * np.sin(
                                                    orient + np.pi / 2 * spot
                                                )
                                        ...
                                        for i in range(iterations):
                                            ...
                                            cube_cen_sub = cube_rescaling(
                                                cube_cen,
                                                scaling_list=scale_list,
                                                ref_xy=ref_xy,
                                                imlib="opencv",
                                            )
                                            if not use_cen_only:
                                                # rescale both the sci and cen cubes using theoretical scaling factors
                                                cube_near = cube_rescaling(
                                                    cube_near,
                                                    scaling_list=scale_list,
                                                    ref_xy=ref_xy,
                                                    imlib="opencv",
                                            ...
                                            cube_cen_sub = np.median(
                                                cube_cen_sub, axis=0
                                            )  # median collapse channels
                                            # find center location
                                            res = frame_center_satspots(
                                                cube_cen_sub,
                                                (
                                                    spot_xy[-1][0],
                                                    spot_xy[-1][-1],
                                            ...
                                            ref_xy = (
                                                frame_center(cube_cen)[1]
                                                - x_shifts_cen_tmp[cc],
                                                frame_center(cube_cen)[0]
                                                - y_shifts_cen_tmp[cc],
                                            )
                                        ...
                                        if plot and cc == 0:
                                            ...
                                            for coords in spot_xy:
                                                x, y = coords.T
                                                ax.scatter(
                                                    x, y, c="white", alpha=0.2
                                                )

                                            plt.savefig(
                                                outpath
                                                + "Predicted_satspots.pdf",
                                                bbox_inches="tight",
                                                pad_inches=0.01,
                                            )
                                            plt.close("all")
                                    ...
                                    write_fits(
                                        outpath + "all_cen_subtracted.fits",
                                        all_cen_sub,
                                        verbose=debug,
                                    )
                                    # median combine results for all MJD CEN bef and all after SCI obs
                                    header_ini = open_header(
                                        inpath + OBJ_IFS_list[0] + ".fits"
                                    )
                                    mjd = float(
                                        header_ini["MJD-OBS"]
                                    )  # mjd of first obs
                                ...
                                if not use_cen_only:  # APPLY THEM TO OBJ CUBES
                                    ...
                                    if plot and fn == 0:
                                        ...
                                        fig.suptitle(
                                            "Shifts inferred from satellite spots"
                                        )
                                        colors = [
                                            "r",
                                            "b",
                                            ...
                                            "g",
                                            "k",
                                        ]  # different colours for each CEN
                                        # y
                                        for cc in range(true_ncen):
                                            axs[0].errorbar(
                                                range(n_z),
                                                y_shifts_cen[cc],
                                                yerr=y_shifts_cen_err[cc],
                                                fmt=colors[cc] + "o",
                                                label=f"y-shifts cube {cc+1}",
                                            )
                                        # x
                                        for cc in range(true_ncen):
                                            axs[1].errorbar(
                                                range(n_z),
                                                x_shifts_cen[cc],
                                                yerr=x_shifts_cen_err[cc],
                                                fmt=colors[cc] + "o",
                                                label=f"x-shifts cube {cc+1}",
                                            )
                                        ...
                                        plt.savefig(
                                            outpath
                                            + "Satspot_shifts_per_channel.pdf",
                                            bbox_inches="tight",
                                        )
                                        plt.close("all")
                                ...
                                if fn == 0 and scaling_from_satspots:
                                    spot_xy_predict = np.zeros(
                                        (len(lbdas), 4, 2)
                                    )  # fill with predictions
                                    spot_xcenters = np.zeros(
                                        (len(lbdas), 4)
                                    )  # measured
                                    spot_ycenters = np.zeros(
                                        (len(lbdas), 4)
                                    )  # measured
                                    snr_val = np.zeros((len(lbdas), 4))
                                    snr_channel_mean = np.zeros(
                                        (ncen, len(lbdas))
                                    )
                                    scale_list_measured_mean = np.zeros(
                                        (ncen, len(lbdas))
                                    )
                                    ...
                                    for cc in range(ncen):
                                        ...
                                        for idx in range(len(lbdas)):
                                            ...
                                            if debug:
                                                print(
                                                    f"Attempting to fit the satellite spot positions in channel {idx}"
                                                )
                                            (
                                                _,
                                                _,
                                                ...
                                                img[idx],
                                                xy=(
                                                    spot_xy_predict[idx][0],
                                                    spot_xy_predict[idx][-1],
                                                    spot_xy_predict[idx][1],
                                                    spot_xy_predict[idx][2],
                                                ),
                                                subi_size=21,
                                                shift=True,
                                                fit_type="moff",
                                        ...
                                        coordinates_array = np.column_stack(
                                            (
                                                spot_xcenters.flatten(),
                                                spot_ycenters.flatten(),
                                            )
                                        )  # stack the columns of x and y coordinates together
                                        coordinates_array = coordinates_array.reshape(
                                            (len(lbdas), 4, 2)
                                        )  # reshape to sames as predicted array
                                        # measure SNR, avoid other spots
                                        for idx in range(len(lbdas)):
                                            for spot in range(4):
                                                try:  # to get the SNR, if it fails then set to nan
                                                    snr_val[idx][spot] = snr(
                                                        img[idx],
                                                        source_xy=(
                                                            coordinates_array[
                                                                idx
                                        ...
                                        snr_channel_mean[cc] = np.nanmean(
                                            snr_val, axis=1
                                        )  # mean of each spot
                                        scale_list_measured_mean[cc] = (
                                            scaling_by_satspots(
                                                lbdas,
                                                coordinates_array,
                                                snr_channel_mean[cc],
                                    ...
                                    snr_channel_mean = np.nanmean(
                                        snr_channel_mean, axis=0
                                    )  # mean of each cube
                                    scale_list_measured_mean = np.nanmean(
                                        scale_list_measured_mean, axis=0
                                    )
                                    write_fits(
                                        outpath + final_scalefac_name,
                                        scale_list_measured_mean,
                                        verbose=debug,
                                    )
                                    ...
                                    write_fits(
                                        outpath + "Satspot_SNR_best.fits",
                                        best,
                                        verbose=debug,
                                    )
                                    print(
                                        f"Five highest SNR channels {best+1} which are {lbdas[best]} µm"
                                    )

                            elif "radon" in rec_met_tmp:
                                cube, y_shifts, x_shifts, _ = (
                                    cube_recenter_radon(
                                        cube,
                                        full_output=True,
                                        verbose=True,
                    ...
                    if plot:
                        try:
                            ...
                            if "satspot" in rec_met_tmp:
                                ax1.errorbar(
                                    unique_mjd_cen,
                                    np.median(y_shifts_cen, axis=1),
                                    np.std(y_shifts_cen, axis=1),
                                    fmt="co",
                                    label="y cen",
                                )
                                ax1.errorbar(
                                    unique_mjd_cen,
                                    np.median(x_shifts_cen, axis=1),
                                    np.std(x_shifts_cen, axis=1),
                                    fmt="mo",
                                    label="x cen",
                                )
                            plt.legend(loc="best")
                            ...
                            print(
                                "Could not produce shifts vs. time plot.",
                                flush=True,
                            )
                            plt.close("all")
        ...
        if 3 in to_do:
            ...
            for fi, file_list in enumerate(obj_psf_list):
                ...
                if (
                    not isfile(
                        outpath + "1_master_ASDIcube{}.fits".format(labels[fi])
                    )
                    or not isfile(outpath + "1_master_derot_angles.fits")
                    or overwrite[2]
                ):
                    if fi != 1:
                        parang_st = []
                    ...
                    write_fits(
                        outpath
                        + "1_master_ASDIcube{}.fits".format(labels[fi]),
                        master_cube,  # /dits[fi],
                        verbose=debug,
                    )
                    ...
                    if fi != 1:
                        ...
                        true_ndit = open_fits(
                            inpath + f"../fits/true_ndit_{labels2[fi]}.fits",
                            verbose=debug,
                            precision=int,
                        )
                        ...
                        for nn, ndit in enumerate(true_ndit):
                            ...
                            y = parang_nd[counter]
                            parang = (
                                x + (y - x) * (0.5 + np.arange(ndit)) / ndit
                            )
                            final_derot_angles.extend(
                                list(parang + TN + pup_off + ifs_off)
                            )
                        ...
                        write_fits(
                            ...
                            np.array(final_derot_angles),
                            verbose=debug,
                        )
                        write_fits(
                            outpath
                            + "1_master_par_angles{}.fits".format(labels[fi]),
                            np.array(final_par_angles),
                            verbose=debug,
                        )
        ...
        if 4 in to_do:
            ...
            for fi, file_list in enumerate(obj_psf_list):
                ...
                if fi == 0 and use_cen_only:
                    ...
                    fi == 2 and not use_cen_only
                ):  # no need for CEN, except if no OBJ
                    break
                ...
                # Determine fwhm
                if (
                    not isfile(outpath + f"TMP_fwhm{labels[fi_tmp]}.fits")
                    or overwrite[3]
                ):
                    cube = open_fits(
                        outpath + f"1_master_ASDIcube{labels[fi_tmp]}.fits",
                        verbose=debug,
                    )
                    fwhm = np.zeros(n_z)
                    ...
                    if isinstance(final_crop_sz_psf, list):
                        final_crop_sz_psf_tmp = final_crop_sz_psf[
                            [
                                i
                                for i in range(len(final_crop_sz_psf))
                                if i % 2 == 0
                            ][-1]
                        ]
                    else:
                    ...
                    # first fit on median
                    for zz in range(n_z):
                        fwhm[zz] = normalize_psf(
                            np.median(cube[zz], axis=0),
                            fwhm="fit",
                            size=final_crop_sz_psf_tmp,
                            threshold=None,
                            mask_core=None,
                            model=psf_model,
                            imlib="opencv",
                    ...
                    write_fits(
                        outpath + f"TMP_fwhm{labels[fi_tmp]}.fits",
                        fwhm,
                        verbose=debug,
                    )
                else:
                ...
                cube = open_fits(
                    outpath + f"1_master_ASDIcube{labels[fi]}.fits",
                    verbose=debug,
                )
                ...
                if (
                    not isfile(
                        outpath
                        + f"2_master{labels[fi]}_ASDIcube_clean_{bad_str}.fits"
                    )
                    or overwrite[3]
                ):
                    # OBJECT
                    if fi != 1:
                        derot_angles = open_fits(
                            outpath
                            + f"1_master_derot_angles{labels[fi]}.fits",
                            verbose=debug,
                        )
                    ...
                    if len(badfr_idx[fi]) > 0:
                        cube_tmp = np.zeros(
                            [
                                cube.shape[0],
                                cube.shape[1] - len(badfr_idx[fi]),
                                cube.shape[2],
                                cube.shape[3],
                            ]
                        )
                        derot_angles_tmp = np.zeros(
                            cube.shape[1] - len(badfr_idx[fi])
                        )
                        counter = 0
                    ...
                    if isfile(outpath + "Satspot_SNR_best.fits"):
                        channels = open_fits(
                            outpath + "Satspot_SNR_best.fits", verbose=debug
                        )
                        channels = np.array(
                            channels, dtype=int
                        )  # ensure channels are integers
                        print(
                            f"Using five highest SNR channels {channels+1} from satspots for bad frame detection"
                        )
                    else:
                    ...
                    for (
                        ...
                        channels
                    ):  # zero based in the case of no prior good channels
                        print(
                            f"********** Identifying bad frames using channel {zz+1} ***********\n",
                            flush=True,
                        )
                        ...
                        plot_tmp = False
                        if (
                            zz == channels[0] or zz == channels[-1] or debug
                        ):  # only plot for channel 1&39 or when debug is on
                            plot_tmp = plot
                        ...
                        if "stat" in badfr_critn_tmp:
                            ...
                            if "rad" in badfr_crit_tmp[idx_stat].keys():
                                rad = int(
                                    badfr_crit_tmp[idx_stat]["rad"] * fwhm_med
                                )
                            if "width" in badfr_crit_tmp[idx_stat].keys():
                                width = int(
                                    badfr_crit_tmp[idx_stat]["width"]
                                    * fwhm_med
                                )
                            ...
                            good_index_list, bad_index_list = (
                                cube_detect_badfr_pxstats(
                                    cube[zz],
                                    mode=mode,
                                    in_radius=rad,
                            ...
                            if plot_tmp:
                                plt.savefig(
                                    outpath
                                    + f"badfr_stat_plot{labels[fi]}_ch{zz}.pdf",
                                    bbox_inches="tight",
                                )
                            final_good_index_list = [
                                idx
                                for idx in list(good_index_list)
                                if idx in final_good_index_list
                            ]
                        ...
                        if "ell" in badfr_critn_tmp:
                            ...
                            if plot_tmp:
                                plt.savefig(
                                    outpath
                                    + f"badfr_ell_plot{labels[fi]}_ch{zz}.pdf",
                                    bbox_inches="tight",
                                )
                            final_good_index_list = [
                                idx
                        ...
                        if "corr" in badfr_critn_tmp:
                            ...
                            if ref == "median":
                                good_frame = np.median(
                                    cube[zz][final_good_index_list], axis=0
                                )
                            else:
                                good_frame = cube[
                                    zz, badfr_crit_tmp[idx_corr]["ref"]
                                ]
                            ...
                            if plot_tmp:
                                plt.savefig(
                                    outpath
                                    + f"badfr_corr_plot{labels[fi]}_ch{zz}.pdf",
                                    bbox_inches="tight",
                                )
                            final_good_index_list = [
                                idx
                        ...
                        print(
                            f"At the end of channel {zz+1}, we kept {len(final_good_index_list)}/{ngood_fr_ch} ({100*(len(final_good_index_list)/ngood_fr_ch):.0f}%) frames\n",
                            flush=True,
                        )

                    if (
                        "corr" in badfr_critn_tmp and debug
                    ):  # save plots of all reference frames in the case of correlation
                        reference_frames = cube_crop_frames(
                            np.median(cube, axis=1),
                            size=crop_size,
                            verbose=debug,
                        )
                        log = False
                        if (
                            not coro or fi == 1
                        ):  # if no coronagraph the image needs to be log scale
                            log = True
                        plot_frames(
                            ...
                            log=log,
                            label=tuple(
                                ["Channel " + str(x) for x in range(1, 40)]
                            ),
                            save=outpath
                            + f"badfr_corr_all_ref_frames{labels[fi]}.pdf",
                        )
                        plt.close("all")
                    ...
                    if fi != 1:
                        ...
                        write_fits(
                            outpath
                            + f"2_master_derot_angles_clean_{bad_str}.fits",
                            derot_angles,
                            verbose=debug,
                        )
            ...
            for fi, file_list in enumerate(obj_psf_list):
                ...
                print(
                    "In total we keep {:.1f}% of all frames of the {} cube \n".format(
                        100 * frac_good, labels[fi]
                    ),
                    flush=True,
                )
        ...
        # ************************* FINAL PSF + FLUX + FWHM ************************
        if 5 in to_do and (
            not isfile(outpath + final_fwhmname + ".fits") or overwrite[4]
        ):
            print(
                "************* 5. FINAL PSF + FLUX + FWHM *************",
                flush=True,
            )
            ...
            if npsf > 0:
                ...
                cube = open_fits(
                    outpath
                    + "2_master_psf_ASDIcube_clean_{}.fits".format(
                        "-".join(badfr_criteria_psf)
                    ),
                    verbose=debug,
                )
                ...
                cube = open_fits(
                    outpath
                    + "2_master_ASDIcube_clean_{}.fits".format(
                        "-".join(badfr_criteria)
                    ),
                    verbose=debug,
                )
                ...
                raise FileNotFoundError(
                    "No suitable frames for PSF + FLUX + FWHM"
                )
            ...
            for crop_sz in crop_sz_list:
                ...
                for zz in range(n_z):
                    ...
                    norm_psf[zz] = norm_res[0]
                    med_flux[zz] = norm_res[1][0]
                    fwhm[zz] = norm_res[2][0]

                write_fits(
                    outpath
                    + "3_final{}_med_{}{:.0f}.fits".format(
                        label_step5, psf_model, crop_sz
                    ),
                    med_psf,
                    verbose=debug,
                )
                write_fits(
                    outpath
                    + "3_final{}_med_{}_norm{:.0f}.fits".format(
                        label_step5, psf_model, crop_sz
                    ),
                    norm_psf,
                    verbose=debug,
                ...
                write_fits(
                    outpath
                    + "3_final{}_flux_med_{}{:.0f}.fits".format(
                        label_step5, psf_model, crop_sz
                    ),
                    np.array([med_flux]),
                    verbose=debug,
                ...
                write_fits(
                    outpath
                    + "3_final{}_fwhm_{}.fits".format(label_step5, psf_model),
                    np.array([fwhm]),
                    verbose=debug,
                )
                if crop_sz % 2:
                    write_fits(
                        outpath + final_psfname + ".fits",
                        med_psf,
                        verbose=debug,
                    )
                    write_fits(
                        outpath + final_psfname_norm + ".fits",
                        norm_psf,
                        verbose=debug,
                    )
                    header = fits.Header()
                    ...
                    write_fits(
                        outpath + final_fluxname + ".fits",
                        np.array(
                            [
                                med_flux * dit_ifs / dit_psf_ifs,
                                med_flux,
                                med_flux / dit_psf_ifs,
                            ]
                        ),
                        header=header,
                        verbose=debug,
                    ...
                    write_fits(
                        outpath + final_fwhmname + ".fits", fwhm, verbose=debug
                    )
                ...
                write_fits(
                    outpath + "3_final{}_fluxes.fits".format(label_step5),
                    fluxes,
                    verbose=debug,
                )
        ...
        if 6 in to_do:
            print(
                "************* 6. FINAL OBJ CUBE (BIN IF NECESSARY) *************",
                flush=True,
            )
            ...
            for cc, crop_sz in enumerate(crop_sz_list):
                ...
                if (
                    ...
                    cube_notrim = open_fits(
                        outpath
                        + "1_master_ASDIcube{}.fits".format(labels[fi_tmp]),
                        verbose=debug,
                    )
                    cube = open_fits(
                        outpath
                        + "2_master{}_ASDIcube_clean_{}.fits".format(
                            labels[fi_tmp], "-".join(badfr_criteria)
                        ),
                        verbose=debug,
                    )
                    derot_angles = open_fits(
                        outpath
                        + "2_master_derot_angles_clean_{}.fits".format(
                            "-".join(badfr_criteria)
                        ),
                        verbose=debug,
                    )
                    ...
                    if bin_fac != 1 and bin_fac is not None and bin_fac != 0:
                        cube, derot_angles = cube_subsample(
                            cube,
                            bin_fac,
                            mode="median",
                            parallactic=derot_angles,
                            verbose=debug,
                        )
                        cube_notrim, derot_angles_notrim = cube_subsample(
                            cube_notrim,
                            bin_fac,
                            mode="median",
                    ...
                    if cube.shape[-2] > crop_sz or cube.shape[-1] > crop_sz:
                        ...
                        cube_notrim = cube_crop_frames(
                            cube_notrim, crop_sz, verbose=debug
                        )
                    flux = open_fits(
                        outpath + final_fluxname + ".fits", verbose=debug
                    )
                    ...
                    if crop_sz % 2:
                        write_fits(
                            outpath + final_cubename + ".fits",
                            cube,
                            verbose=debug,
                        )
                        write_fits(
                            outpath + final_cubename_norm + ".fits",
                            cube_norm,
                            verbose=debug,
                        )
                        write_fits(
                            outpath + final_anglename + ".fits",
                            derot_angles,
                            verbose=debug,
                        )
                        if plot:
                            ...
                            plt.plot(
                                derot_angles
                            )  # plot the first one from each cube
                            plt.xlabel("Science cube")
                            plt.ylabel("Derotation angle [deg]")
                            plt.minorticks_on()
                            plt.grid(alpha=0.1)
                            plt.savefig(
                                outpath + "Derotation_angle_vector.pdf",
                                bbox_inches="tight",
                                pad_inches=0.1,
                            )
                            plt.close("all")
                    write_fits(
                        outpath
                        + "3_final_cube_all_bin{:.0f}_{:.0f}.fits".format(
                            bin_fac, crop_sz
                        ),
                        cube_notrim,
                        verbose=debug,
                    ...
                    write_fits(
                        outpath
                        + "3_final_cube_all_bin{:.0f}_{:.0f}_norm.fits".format(
                            bin_fac, crop_sz
                        ),
                        cube_notrim_norm,
                        verbose=debug,
                    ...
                    write_fits(
                        outpath
                        + "3_final_derot_angles_all_bin{:.0f}.fits".format(
                            bin_fac
                        ),
                        derot_angles_notrim,
                        verbose=debug,
                    ...
                    write_fits(
                        outpath
                        + "3_final_cube_bin{:.0f}_{:.0f}.fits".format(
                            bin_fac, crop_sz
                        ),
                        cube,
                        verbose=debug,
                    ...
                    write_fits(
                        outpath
                        + "3_final_cube_bin{:.0f}_{:.0f}_norm.fits".format(
                            bin_fac, crop_sz
                        ),
                        cube_norm,
                        verbose=debug,
                    ...
                    write_fits(
                        outpath
                        + "3_final_derot_angles_bin{:.0f}.fits".format(
                            bin_fac
                        ),
                        derot_angles,
                        verbose=debug,
        ...
        if 7 in to_do:
            print(
                "************* 7. FINAL OBJ ADI CUBES and PSF frames *************",
                flush=True,
            )
            # OBJs
            if isinstance(final_crop_szs[0], (float, int)):
            ...
            if (
                not isfile(
                    outpath_ADIcubes + "ADI_cube_ch{:.0f}.fits".format(n_z - 1)
                )
                or overwrite[6]
            ):
                # for bb, bin_fac in enumerate(bin_fac_list):
                for cc, crop_sz in enumerate(crop_sz_list):
                    ...
                    for ff in range(n_z):
                        write_fits(
                            outpath_ADIcubes
                            + "ADI_cube_ch{:.0f}_bin{:.0f}_{:.0f}.fits".format(
                                ff, bin_fac, crop_sz
                            ),
                            ASDI_cube[ff],
                            verbose=debug,
            ...
            if npsf > 0:
                ...
                if (
                    not isfile(
                        outpath_PSFframes
                        + "PSF_frame_ch{:.0f}.fits".format(n_z - 1)
                    )
                    or overwrite[6]
                ):
                    # for bb, bin_fac in enumerate(bin_fac_list):
                    for cc, crop_sz in enumerate(crop_sz_list):
                        ...
                        PSF_cube = open_fits(
                            outpath
                            + "3_final_psf_med_{}_norm{:.0f}.fits".format(
                                psf_model, crop_sz
                            ),
                            verbose=debug,
                        )
                        for ff in range(n_z):
                            write_fits(
                                outpath_PSFframes
                                + "PSF_frame_ch{:.0f}_{:.0f}.fits".format(
                                    ff, crop_sz
                                ),
                                PSF_cube[ff],
                                verbose=debug,
        ...
        if 8 in to_do:
            ...
            derot_angles = open_fits(
                outpath + final_anglename + ".fits", verbose=debug
            )
            ...
            for ff in range(n_z - 1):
                cube_ff = open_fits(
                    outpath + final_cubename + ".fits", verbose=debug
                )[ff]
                cube_last = open_fits(
                    outpath + final_cubename + ".fits", verbose=debug
                )[-1]
                ...
                if ff == 0:
                    if isinstance(mask_scal, str):
                        ...
                        if mask_scal[0]:
                            if mask_scal[1]:
                                mask_scal = get_annulus_segments(
                                    mask,
                                    mask_scal[0] / plsc,
                                    mask_scal[1] / plsc,
                                    nsegm=1,
                                ...
                                mask_scal = mask_circle(
                                    mask, mask_scal[0] / plsc
                                )
                        if debug:
                            write_fits(
                                outpath + "TMP_mask_scal.fits",
                                mask_scal,
                                verbose=debug,
                            )
            ...
            for i in range(n_cubes):
                ...
                resc_cube_res = np.zeros(
                    [lg - fg + 1, master_cube.shape[-2], master_cube.shape[-1]]
                )
                ...
                write_fits(
                    outpath
                    + "TMP_resc_cube{:.0f}_res_{:.0f}fp.fits".format(i, nfp),
                    resc_cube_res_i,
                    verbose=debug,
                )
                resc_cube_res[-1] = tmp2 - tmp1
                write_fits(
                    outpath + "TMP_resc_cube_{:.0f}fp.fits".format(nfp),
                    resc_cube,
                    verbose=debug,
                )
            ...
            for nn, n_med in enumerate(n_med_sdi):
                resc_cube_res_all = []
                for i in range(n_cubes):
                    resc_cube_res_all.append(
                        open_fits(
                            outpath
                            + "TMP_resc_cube{:.0f}_res_{:.0f}fp.fits".format(
                                i, nfp
                            ),
                            verbose=debug,
                        )[nn]
                ...
                write_fits(
                    outpath
                    + "final_simple_SDI_{:.0f}fp_nmed{:.0f}.fits".format(
                        nfp, n_med
                    ),
                ...
                write_fits(
                    outpath
                    + "final_simple_SDI_stim_{:.0f}fp_nmed{:.0f}.fits".format(
                        nfp, n_med
                    ),
                    stim_maps,
                    verbose=debug,
            ...
            print(
                "final flux fac result ({:.0f}): ".format(nfp),
                flux_fac_vec,
                flush=True,
            )
            write_fits(
                outpath + final_scalefac_name, scal_vector, verbose=debug
            )
            write_fits(
                outpath + "final_flux_fac.fits", flux_fac_vec, verbose=debug
            )
    return None
