...
#    fwhm_name = final_fwhmname
    ...
    delta_rot_ann = params_postproc.get('delta_rot_ann',[1,4]) 
    delta_rot_ann = tuple(delta_rot_ann)
    ...
    label_test_tmp =  '_mask{:.1f}_deltarot{:.1f}_maxfr{:.0f}' # default format if no label test provided
    ...
    if coro:
        transmission_name = vcal_path[0] + "/../Filters/" + "SPHERE_IRDIS_ALC_transmission_px.fits"
    ...
    PCA_ASDI_cube_ori= vip.fits.open_fits(inpath+final_cubename)
    nz, nn, ny, nx = PCA_ASDI_cube_ori.shape
    
    derot_angles = vip.fits.open_fits(inpath+final_anglename)
    lbdas = vip.fits.open_fits(inpath+final_lbdaname)
    ...
    if subtract_planet:
        PCA_ASDI_cube_ori = vip.negfc.cube_planet_free(planet_parameter, 
                                                   PCA_ASDI_cube_ori, derot_angles, 
                                                   psfn, plsc)
    if planet or fake_planet:
        cy, cx = vip.var.frame_center(PCA_ASDI_cube_ori[0,0])
    if planet:
    ...
    for max_fr in max_fr_list:         
        ...
        if fake_planet:
            if not isfile(outpath+'TMP_first_guess_5sig_sensitivity_ASDI'+label_test+'.fits') or not isfile(outpath+'TMP_optimal_contrast_curve_PCA-ASDI-full_randsvd.csv') or overwrite_pp:
                ...
                for nn, npc in enumerate(firstguess_pcs):
                    pn_contr_curve_full_rr = vip.metrics.contrast_curve(PCA_ASDI_cube_ori, derot_angles, psfn,
                                                                        fwhm, plsc, starphot=starphot, 
                                                                        algo=vip.pca.pca, sigma=5., nbranch=1, 
        ...
        if fake_planet:
            ...
            for ns in range(nspi):
                ...
                for ff in range(nfcp):
                    ...
                    PCA_ASDI_cube = vip.metrics.cube_inject_companions(PCA_ASDI_cube, psfn, 
                                                                       derot_angles, flevel, 
            ...
            if not fake_planet:
                if not isfile(outpath+'final_PCA-SDI_'+test_pcs_str+label_test+'.fits') or overwrite_pp:
                    ...
                    for pp, npc in enumerate(test_pcs_sdi):
                        tmp[pp], tmp_tmp, tmp_tmp_der = pca(PCA_ASDI_cube, angle_list=derot_angles, cube_ref=None, scale_list=scale_list, 
                        ...
                        if planet:
                            snr_tmp[pp] = vip.metrics.snr(tmp[pp], (xx_comp,yy_comp), fwhm_med, plot=False, exclude_negative_lobes=True,
                                                          verbose=False)
                ...
                if (not isfile(outpath+'final_PCA-SDI_'+test_pcs_str+label_test+'_snrmap.fits') or overwrite_pp) and do_snr_map:
                    ...
                    for pp in range(tmp.shape[0]):
                        ...
                        tmp[pp] = vip.var.mask_circle(tmp[pp],rad_in*fwhm_med)
                ...
                for ns in range(nspi):
                    theta0 = th0+ns*th_step
                    PCA_ASDI_cube = vip.fits.open_fits(outpath+'PCA_cube'+label_test+'_fcp_spi{:.0f}.fits'.format(ns))
                    for pp, npc in enumerate(test_pcs_sdi):
                        if svd_mode == 'randsvd':
                        ...
                        for ff in range(nfcp):
                            ...
                            snr_tmp_tmp[ns,pp,ff] = vip.metrics.snr(tmp_tmp[pp], (xx_fcp,yy_fcp), fwhm_med, plot=False, exclude_negative_lobes=True,
                                                                  verbose=True)   
        ...
        if do_adi_full:
            ...
            if not fake_planet:
                ...
                for pp, npc in enumerate(test_pcs_adi_full):
                    for zz in range(nz):
                        tmp[pp,zz] = pca(PCA_ASDI_cube[zz], angle_list=derot_angles, 
                        ...
                        if planet:
                            snr_tmp[pp,zz] = vip.metrics.snr(tmp[pp,zz], (xx_comp,yy_comp), fwhm[zz], plot=False, exclude_negative_lobes=True,
                                                          verbose=False)
                            if zz ==0:
                ...
                if planet:
                    ...
                    plt.xlabel('npc')
                    for pp, npc in enumerate(test_pcs_adi_full):
                        snr_tmp[pp] = vip.metrics.snr(tmp_tmp[pp], (xx_comp,yy_comp), fwhm_med, plot=False, exclude_negative_lobes=True,
                                                      verbose=False)
                        if snr_tmp[pp] > 5:
                            marker = 'go'
                        elif snr_tmp[pp] > 3:
                ...
                for ns in range(nspi):
                    ...
                    for pp, npc in enumerate(test_pcs_adi_full):
                        ...
                        for ff in range(nfcp):
                            xx_fcp = cx + rad_arr[ff]*np.cos(np.deg2rad(theta0+ff*th_step))
                            yy_fcp = cy + rad_arr[ff]*np.sin(np.deg2rad(theta0+ff*th_step))
                            snr_tmp[ns,pp,ff] = vip.metrics.snr(tmp_tmp[pp], (xx_fcp,yy_fcp), fwhm_med, plot=False, exclude_negative_lobes=True,
                                                                  verbose=True)   
        ...
        if do_adi_ann:
            ...
            if not fake_planet:
                ...
                for pp, npc in enumerate(test_pcs_adi_ann):
                    if not isfile(outpath+'PCA-ADI_ann_npc{:.0f}'.format(npc)+label_test+'.fits') or overwrite_pp:
                        for zz in range(start_nz,nz):
                            tmp[pp,zz] = pca_annular(PCA_ASDI_cube[zz], derot_angles, radius_int=mask_IWA_px, fwhm=fwhm_med, asize=asize, 
                                                  delta_rot=delta_rot_ann, ncomp=int(npc), svd_mode=svd_mode, scale_list=None,
                        ...
                        if planet:
                            ...
                            write_fits(outpath+'PCA-ADI_ann_SNR_npc{:.0f}'.format(npc)+label_test+'.fits', snr_tmp)
                        write_fits(outpath+'PCA-ADI_ann_npc{:.0f}'.format(npc)+label_test+'.fits', tmp[pp])  
                ...
                if planet:
                    ...
                    write_fits(outpath+'final_PCA-ADI_ann_opt_npc_planet{}.fits'.format(label_test), tmp_tmp_tmp)
                    tmp=None
                    tmp_tmp_tmp = None
                    write_fits(outpath+'PCA-ADI_ann_OPT_NPC_planet{}.fits'.format(label_test), opt_npc)
                    ...
                    plt.xlabel('npc')
                    for pp, npc in enumerate(test_pcs_adi_ann):
                        snr_tmp[pp] = vip.metrics.snr(tmp_tmp[pp], (xx_comp,yy_comp), 
                    ...
                    write_fits(outpath+'PCA-ADI_ann_collapsed_SNR{}.fits'.format(label_test), snr_tmp)
                    write_fits(outpath+'final_PCA-ADI_ann_collapsed_opt_npc{:.0f}_planet{}.fits'.format(npc_opt,label_test), tmp_tmp[opt_pp])  
                write_fits(outpath+'PCA-ADI_ann_collapsed_{}{}.fits'.format(test_pcs_str,label_test), tmp_tmp) 
            else:
                snr_tmp = np.zeros([nspi,ntest_pcs,nfcp])
                tmp_tmp = np.zeros([ntest_pcs,PCA_ASDI_cube.shape[2],PCA_ASDI_cube.shape[3]])
                for ns in range(nspi):
                    ...
                    for pp, npc in enumerate(test_pcs_adi_ann):
                        for zz in range(start_nz,nz):
                            tmp[zz] = pca_annular(PCA_ASDI_cube[zz], derot_angles, radius_int=mask_IWA_px, fwhm=fwhm_med, asize=asize, 
                                              delta_rot=delta_rot_ann, ncomp=int(npc), svd_mode=svd_mode, scale_list=None,
                                              min_frames_lib=max(npc,10), max_frames_lib=max(max_fr,npc+1), collapse='median', full_output=False, 
                                              verbose=verbose, nproc=nproc)
                        if debug:
                            write_fits(outpath+'TMP_PCA-ADI_ann_npc{:.0f}{}_fcp_spi{:.0f}.fits'.format(npc,label_test,ns), tmp)
                ...
                write_fits(outpath+'PCA-ADI_ann_SNR_fcps_'+test_pcs_str+label_test+'.fits', snr_fcp)   
                ...
                for pp, npc in enumerate(id_npc_adi_ann_df):
                    for zz in range(start_nz,nz):
                        tmp[zz] = pca_annular(PCA_ASDI_cube_ori[zz], derot_angles, radius_int=mask_IWA_px, fwhm=fwhm_med, asize=asize, 
                                              delta_rot=delta_rot_ann, ncomp=int(npc), svd_mode=svd_mode, scale_list=None,
                                              min_frames_lib=max(npc,10), max_frames_lib=max(max_fr,npc+1), collapse='median', full_output=False, 
                                              verbose=verbose, nproc=nproc)
                    if debug:
                        write_fits(outpath+'TMP_PCA-ADI_ann_opt_npc{:.0f}{}_fcp_spi{:.0f}.fits'.format(npc,label_test,ns), tmp)
                    tmp_tmp[pp] = np.median(tmp[start_nz:], axis=0)            
                write_fits(outpath+'final_PCA-ADI_ann_image_{}_at_{}as'.format(test_pcs_str,test_rad_str)+label_test+'.fits', tmp_tmp)
                write_fits(outpath+'final_PCA-ADI_ann_opt_npc_at_{}as'.format(test_rad_str)+label_test+'.fits', id_npc_adi_ann_df)           
                
                ### SNR map  
                if (not isfile(outpath+'final_PCA-ADI_ann_snrmap_{}_at_{}as'.format(test_pcs_str,test_rad_str)+label_test+'.fits') or overwrite_pp) and do_snr_map:
                    tmp = open_fits(outpath+'final_PCA-ADI_ann_image_{}_at_{}as'.format(test_pcs_str,test_rad_str)+label_test+'.fits')
                    rad_in = mask_IWA # we comment it for a better visualization of the snr map (there are spurious values in the center)
                    #rad_in = 1.5
                    #tmp_tmp = np.ones_like(tmp)
                    for pp in range(tmp.shape[0]):
                    ...
                    if not fake_planet:     
                        ...
                        for pp1, npc1 in enumerate(test_pcs_sadi_full_sdi):
                            for pp2, npc2 in enumerate(test_pcs_sadi_full_adi):
                                tmp[counter], tmp_tmp, tmp_tmp_der = pca(PCA_ASDI_cube, angle_list=derot_angles, cube_ref=None, scale_list=scale_list, 
                                ...
                                if planet:  
                                    snr_tmp[counter] = vip.metrics.snr(tmp_tmp[counter], (xx_comp,yy_comp), fwhm_med, 
                        ...
                        for ns in range(nspi):
                            ...
                            for pp1, npc1 in enumerate(test_pcs_sadi_full_sdi):
                                for pp2, npc2 in enumerate(test_pcs_sadi_full_adi):
                                    tmp_tmp[counter] = pca(PCA_ASDI_cube, angle_list=derot_angles, cube_ref=None, scale_list=scale_list, 
                                                          ncomp=(int(npc1),int(npc2)), svd_mode=svd_mode, scaling=scal, 
                                                          mask_center_px=mask_IWA_px, adimsdi='double', delta_rot=1, fwhm=fwhm_med, 
                                                          collapse='median', check_memory=True, ifs_collapse_range=ifs_collapse_range,
                                                          full_output=False, verbose=verbose, conv=do_conv, mask_rdi=mask_rdi)
                                    for ff in range(nfcp):
                                        xx_fcp = cx + rad_arr[ff]*np.cos(np.deg2rad(theta0+ff*th_step))
                                        yy_fcp = cy + rad_arr[ff]*np.sin(np.deg2rad(theta0+ff*th_step))
                                        snr_tmp_tmp[ns,pp,ff] = vip.metrics.snr(tmp_tmp[counter], (xx_fcp,yy_fcp), fwhm_med, plot=False, exclude_negative_lobes=True,
                                                                              verbose=True)   
                                    counter +=1
                    ...
                    if not fake_planet:
                        ...
                        for pp1, npc1 in enumerate(test_pcs_sadi_ann_sdi):
                            for pp2, npc2 in enumerate(test_pcs_sadi_ann_adi):
                                ...
                                if planet:  
                                    snr_tmp[counter] = vip.metrics.snr(tmp_tmp[counter], (xx_comp,yy_comp), fwhm_med, plot=False, exclude_negative_lobes=True,
                                                                       verbose=False)
                                counter+=1                                          
                        if planet:
                            plt.close() 
                            ...
                            write_fits(outpath+'final_PCA-SADI2_ann_SNR_'+test_pcs_str+label_test+'.fits', snr_tmp)
                        write_fits(outpath+'final_PCA-SADI2_ann_'+test_pcs_str+label_test+'.fits', tmp)
                        if do_stim_map:
                            write_fits(outpath+'final_PCA-SADI2_ann_'+test_pcs_str+label_test+'_stimmap.fits', stim_map)
                            write_fits(outpath+'final_PCA-SADI2_ann_'+test_pcs_str+label_test+'_invstimmap.fits', inv_stim_map)
                            write_fits(outpath+'final_PCA-SADI2_ann_'+test_pcs_str+label_test+'_stimthr.fits', thr)
                    else:
                        snr_tmp_tmp = np.zeros([nspi,ntest_pcs,nfcp])
                        tmp_tmp = np.zeros([ntest_pcs,PCA_ASDI_cube.shape[1],PCA_ASDI_cube.shape[2]])
                        for ns in range(nspi):
                        ...
                        write_fits(outpath+'final_PCA-SADI2_ann_SNR_fcps_'+test_pcs_str+label_test+'.fits', snr_fcp)   
                        ...
                        for pp, idx_npc_ann in enumerate(id_npc_ann_df):
                            tmp_tmp[pp] = pca_annular(PCA_ASDI_cube_ori, derot_angles, radius_int=mask_IWA_px, fwhm=fwhm_med, asize=asize, 
                                                      delta_rot=delta_rot_ann, ncomp=(int(idx_npc_ann[0]),int(idx_npc_ann[1])), scaling=scal,
                                                      svd_mode=svd_mode, scale_list=scale_list, ifs_collapse_range=ifs_collapse_range,
                                                      collapse='median', max_frames_lib=max(max_fr,max(idx_npc_ann)+1), full_output=False, verbose=verbose, nproc=nproc)
                        write_fits(outpath+'final_PCA-SADI2_ann_image_{}_at_{}as'.format(test_pcs_str,test_rad_str)+label_test+'.fits', tmp_tmp)
                        write_fits(outpath+'final_PCA-SADI2_ann_opt_npc_at_{}as'.format(test_rad_str)+label_test+'.fits', id_npc_ann_df)
                
                    
                        ### SNR map  
                        if (not isfile(outpath+'final_PCA-SADI2_ann_snrmap_{}_at_{}as'.format(test_pcs_str,test_rad_str)+label_test+'.fits') or overwrite_pp) and do_snr_map:
                            tmp = open_fits(outpath+'final_PCA-SADI2_ann_image_{}_at_{}as'.format(test_pcs_str,test_rad_str)+label_test+'.fits')
                            rad_in = mask_IWA # we comment it for a better visualization of the snr map (there are spurious values in the center)
                            #rad_in = 1.5
                            tmp_tmp = np.ones_like(tmp)
                            for pp in range(tmp.shape[0]):
        ...
        if planet and planet_parameter is not None:
            ...
            PCA_ASDI_cube_ori = vip.negfc.cube_planet_free(planet_parameter, PCA_ASDI_cube_ori, derot_angles, psfn, plsc)
            label_emp = '_empty'
        else:
            label_emp = ''
        for ss, scal in enumerate(scalings):
            ...
            for ii, ifs_collapse_range in enumerate(ifs_collapse_range_list):
                ...
                if do_sdi and fake_planet:
                    ...
                    for rr, rad in enumerate(rad_arr):
                        if svd_mode == 'randsvd':
                            for nr in range(3):
                                pn_contr_curve_sdi_rr_tmp = vip.metrics.contrast_curve(PCA_ASDI_cube_ori, derot_angles, psfn,
                            ...
                            pn_contr_curve_sdi_rr = vip.metrics.contrast_curve(PCA_ASDI_cube_ori, derot_angles, psfn,
                                                                               fwhm, plsc, starphot=starphot, scaling=scal,
                                                                               algo=vip.pca.pca, sigma=5., nbranch=1, 
                                                                               theta=0, inner_rad=1, wedge=(0,360),fc_snr=fc_snr,
                                                                               student=True, transmission=transmission, 
                ...
                if do_sadi_ann and fake_planet:   
                    ...
                    df_list = []
                    for rr, rad in enumerate(rad_arr):
                        pn_contr_curve_ann_rr = vip.metrics.contrast_curve(PCA_ASDI_cube_ori, derot_angles, psfn,
                                                      fwhm, plsc, starphot=starphot, 
                                                      algo=vip.pca.pca_annular, sigma=5., nbranch=1, 
                                                      theta=0, inner_rad=1, wedge=(0,360),fc_snr=fc_snr,
                                                      student=True, transmission=transmission, 
