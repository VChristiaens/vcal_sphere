...
def postproc_IFS(
    params_postproc_name="VCAL_params_postproc_IFS.json",
    ...
    *******
    ...
    if local_path:
        f1 = join(vcal_path, "Static/SPHERE_IFS_ALC_transmission_px.fits")
    else:
        try:
            f1 = download_file(
                join(url_d, "SPHERE_IFS_ALC_transmission_px.fits"), cache=True
            )
        except:
    ...
    label_test = params_postproc.get(
        "label_test", ""
    )  # manually provide a test label

    source = sourcename.replace(" ", "")  # same without space

    ## Options
    imlib = params_postproc.get(
        "imlib", "vip-fft"
    )  # image processing library to be used
    ...
    )  # number of processors to use - can also be set to cpu_count()/2 for efficiency
    ...
    do_adi_full = params_postproc.get(
        "do_adi_full", 1
    )  # PCA-ADI in full frame in each spectral channel
    do_adi_ann = params_postproc.get(
        "do_adi_ann", 0
    )  # PCA-ADI in annuli in each spectral channel
    do_sadi_full = params_postproc.get(
        "do_sadi_full", 1
    )  # PCA-SADI in full frame
    do_sadi_ann = params_postproc.get("do_sadi_ann", 0)  # PCA-SADI in annuli

    ## Planet?
    planet = params_postproc.get("planet", 0)  # is there a companion?
    planet_pos = params_postproc.get(
        "planet_pos", None
    )  # If so, where is it (or where is it expected)?   (x, y) in frame
    ...
    fake_planet = params_postproc.get(
        "fake_planet", 0
    )  #  FIRST RUN IT AS FALSE TO CHECK FOR THE PRESENCE OF TRUE COMPANIONS
    ...
    nspi = params_postproc.get(
        "nspi", 9
    )  # number of spirals where fcps should be injected - also corresponds to number of PAs where the contrast curve is computed
    wedge = tuple(
    ...
    mask_IWA_px = params_postproc.get(
        "mask_IWA", 10
    )  # just show pca images beyond the provided mask radius (provide in pixels)
    ...
    crop_ifs = params_postproc.get(
        "crop_ifs", 0
    )  # whether to crop the IFS frames after rescaling during PCA-SDI. Leave it to False for SINFONI.
    scalings = [
        params_postproc.get("scaling", None)
    ]  # list of pre-PCA cube scaling(s) to be tested for PCA-SADI.

    ### PCA options
    delta_rot = params_postproc.get(
        "delta_rot", 1
    )  # float or int expressed in FWHM. Threshold in azimuthal motion to keep frames in the PCA library
    delta_rot_ann = params_postproc.get(
        "delta_rot_ann", [1, 3]
    )  # float, int or tuple expressed in FWHM. Threshold in azimuthal motion to keep frames in the PCA library created by PCA-annular. If a tuple, corresponds to the threshold for the innermost and outermost annuli, respectively.
    if type(delta_rot_ann) == list and len(delta_rot_ann) == 2:
        delta_rot_ann = tuple(
            delta_rot_ann
        )  # converts to tuple as .json parameter file does not support tuples
    delta_sep = params_postproc.get("delta_sep", [0.1, 1])
    asize = params_postproc.get(
        "asize", 3
    )  # width of the annnuli for either pca in concentric annuli or on a single annulus, provided in FWHM
    #### how is SVD done for PCA:
    svd_mode = params_postproc.get("svd_mode", "lapack")
    #### number of principal components
    firstguess_pcs = params_postproc.get(
        "firstguess_pcs", [1, 21, 1]
    )  # explored for first contrast curve
    ...
    test_pcs_sadi_full_sdi = params_postproc.get(
        "pcs_sadi_full_sdi", [1, 4, 1]
    )
    test_pcs_sadi_full_adi = params_postproc.get(
    ...
    test_pcs_sdi = list(
        range(test_pcs_sdi[0], test_pcs_sdi[1], test_pcs_sdi[2])
    )
    test_pcs_adi_full = list(
        range(test_pcs_adi_full[0], test_pcs_adi_full[1], test_pcs_adi_full[2])
    )
    test_pcs_adi_ann = list(
        range(test_pcs_adi_ann[0], test_pcs_adi_ann[1], test_pcs_adi_ann[2])
    )
    test_pcs_sadi_full = list(
        range(
            test_pcs_sadi_full[0], test_pcs_sadi_full[1], test_pcs_sadi_full[2]
        )
    )
    test_pcs_sadi_full_sdi = list(
        range(
            test_pcs_sadi_full_sdi[0],
            test_pcs_sadi_full_sdi[1],
            test_pcs_sadi_full_sdi[2],
        )
    )
    test_pcs_sadi_full_adi = list(
        range(
            test_pcs_sadi_full_adi[0],
            test_pcs_sadi_full_adi[1],
            test_pcs_sadi_full_adi[2],
        )
    )
    test_pcs_sadi_ann_sdi = list(
        range(
            test_pcs_sadi_ann_sdi[0],
            test_pcs_sadi_ann_sdi[1],
            test_pcs_sadi_ann_sdi[2],
        )
    )
    test_pcs_sadi_ann_adi = list(
        range(
            test_pcs_sadi_ann_adi[0],
            test_pcs_sadi_ann_adi[1],
            test_pcs_sadi_ann_adi[2],
        )
    )
    ...
    colors = [
        ...
        "c",
        "y",
    ]  # for plotting of ADI full, ADI ann and SADI ann
    all_markers = [
        "ro",
        "yo",
        "bo",
    ...
    if type(start_nz) and type(end_nz) == int:
        ...
        if (
            end_nz - start_nz + 1 == nz
        ):  # if all channels are to be used (+1 for zero-based indexing)
            ifs_collapse_range_lab = ["all_ch"]
        else:
            ifs_collapse_range_lab = [
                "ch{}-{}".format(start_nz + 1, end_nz + 1)
            ]
        print(
            "Using spectral channels {}-{} for SADI".format(
                start_nz + 1, end_nz + 1
            ),
            flush=True,
        )
        ...
        if len(start_nz) == len(end_nz):
            ...
            for i in range(len(end_nz)):
                ...
                if end_nz[i] - start_nz[i] + 1 == nz:
                    ifs_collapse_range_lab.append(
                        ("all_ch")
                    )  # don't remove redundant parenthesis
                else:
                    ifs_collapse_range_lab.append(
                        ("ch{}-{}".format(start_nz[i] + 1, end_nz[i] + 1))
                    )
                print(
                    "Using spectral channels {}-{} for SADI reduction {}".format(
                        start_nz[i] + 1, end_nz[i] + 1, i + 1
                    ),
                    flush=True,
                )
        else:
            raise TypeError(
                "Lists of start and end spectral channels should be equal length."
            )
    else:
        raise TypeError(
            "Start and end spectral channel should be ints or lists."
        )
    ...
    if final_scalefacname is not None and isfile(
        inpath + final_scalefacname + ".fits"
    ):
    ...
    if subtract_planet:
        PCA_ASDI_cube_ori = cube_planet_free(
            planet_parameter,
            PCA_ASDI_cube_ori,
            derot_angles,
            psfn,
    ...
    for max_fr in max_fr_list:
        ...
        if label_test == "":  # if no custom label is provided, make one
            label_test = "_mask{:.1f}_deltarot{:.1f}_maxfr{:.0f}".format(
                mask_IWA, delta_rot, max_fr
            )
            if type(delta_rot_ann) == tuple:
                label_test_ann = (
                    "_mask{:.1f}_deltarot{:.1f}-{:.1f}_maxfr{:.0f}".format(
                        mask_IWA, delta_rot_ann[0], delta_rot_ann[1], max_fr
                    )
                )
            # if delta_rot_ann is an int or a float, show it in the label to one decimal place
                ...
                label_test_ann = (
                    "_mask{:.1f}_deltarot{:.1f}_maxfr{:.0f}".format(
                        mask_IWA, delta_rot_ann, max_fr
                    )
                )
            else:
                label_test_ann = label_test
        ...
        if fake_planet:
            if (
                not isfile(
                    outpath
                    + "TMP_first_guess_5sig_sensitivity_ASDI"
                    + label_test
                    + ".fits"
                )
                or not isfile(
                    outpath
                    + "TMP_optimal_contrast_curve_PCA-ASDI-full_randsvd.csv"
                )
                or overwrite_pp
            ):
                df_list = []
                ...
                for jj in range(pn_contr_curve_full_rsvd_opt.shape[0]):
                    ...
                    for rr, rad in enumerate(rad_arr):
                        sensitivities.append(
                            df_list[rr]["sensitivity (Student)"][jj]
                        )
                    print(
                        "Sensitivities at {}: ".format(
                            df_list[rr]["distance"][jj]
                        ),
                        sensitivities,
                    )
                    idx_min = np.argmin(sensitivities)
                    pn_contr_curve_full_rsvd_opt["sensitivity (Student)"][
                        jj
                    ] = df_list[idx_min]["sensitivity (Student)"][jj]
                    pn_contr_curve_full_rsvd_opt["sensitivity (Gauss)"][jj] = (
                        df_list[idx_min]["sensitivity (Gauss)"][jj]
                    )
                    pn_contr_curve_full_rsvd_opt["throughput"][jj] = df_list[
                        idx_min
                    ]["throughput"][jj]
                    pn_contr_curve_full_rsvd_opt["noise"][jj] = df_list[
                        idx_min
                    ]["noise"][jj]
                    pn_contr_curve_full_rsvd_opt["sigma corr"][jj] = df_list[
                        idx_min
                    ]["sigma corr"][jj]
                DF.to_csv(
        ...
        if fake_planet:
            ...
            for ns in range(nspi):
                ...
                for ff in range(nfcp):
                    if ff + 1 > sensitivity_5sig_full_rsvd_df.shape[0]:
                        flevel = (
                            ...
                            * sensitivity_5sig_full_rsvd_df[-1]
                            / np.sqrt(((rad_arr[ff] * plsc) / 0.5))
                        )
                    else:
                        flevel = (
                            starphot * sensitivity_5sig_full_rsvd_df[ff]
                    ...
                    PCA_ASDI_cube = cube_inject_companions(
                        PCA_ASDI_cube,
                        psfn,
                        derot_angles,
                        flevel,
            ...
            if not fake_planet:
                if (
                    ...
                    tmp = np.zeros(
                        [
                            ntest_pcs,
                            PCA_ASDI_cube.shape[2],
                            PCA_ASDI_cube.shape[3],
                        ]
                    )
                    if do_stim_map:
                        stim_map = np.zeros(
                    ...
                    for pp, npc in enumerate(test_pcs_sdi):
                        ...
                        tmp[pp], tmp_tmp, tmp_tmp_der = pca(
                            algo_params=params_pca
                        )
                        if do_stim_map:
                            stim_map[pp] = compute_stim_map(tmp_tmp_der)
                            inv_stim_map[pp] = compute_inverse_stim_map(
                                tmp_tmp, derot_angles, nproc=nproc, imlib=imlib
                            )
                ...
                if (
                    ...
                    for pp in range(tmp.shape[0]):
                        tmp[pp] = snrmap(
                            tmp[pp], fwhm_med, plot=False, nproc=nproc
                        )
                ...
                tmp_tmp = np.zeros(
                    [ntest_pcs, PCA_ASDI_cube.shape[2], PCA_ASDI_cube.shape[3]]
                )
                for ns in range(nspi):
                    theta0 = th0 + ns * th_step
                    PCA_ASDI_cube = open_fits(
                        outpath
                        + "PCA_cube"
                        + label_test
                        + "_fcp_spi{:.0f}.fits".format(ns)
                ...
                try:
                    plt.savefig(
                        outpath_fig + "SNR_fcps_PCA-SDI" + ".pdf", format="pdf"
                    )
                except:
                    pass
                write_fits(
                    outpath
                    + "final_PCA-SDI_SNR_fcps_"
                    + test_pcs_str
                    + label_test
                    + ".fits",
                    snr_fcp,
                ...
                for pp, npc in enumerate(id_npc_sdi_df):
                    if svd_mode == "randsvd":
                        ...
                        for nr in range(3):
                            ...
                            tmp_tmp_tmp[nr] = pca(
                                algo_params=params_pca, source_xy=source_xy
                            )
                        tmp_tmp[pp] = np.median(tmp_tmp_tmp, axis=0)
                    else:
                        params_pca = PCA_Params(
                ...
                write_fits(
                    outpath
                    + "final_PCA-SDI_image_{}_at_{}as".format(
                        test_pcs_str, test_rad_str
                    )
                    + label_test
                    + ".fits",
                    tmp_tmp,
                ...
                write_fits(
                    outpath
                    + "final_PCA-SDI_opt_npc_at_{}as".format(test_rad_str)
                    + label_test
                    + ".fits",
                    id_npc_sdi_df,
                )
                ...
                if (
                    not isfile(
                        outpath
                        + "final_PCA-SDI_snrmap_{}_at_{}as".format(
                            test_pcs_str, test_rad_str
                        )
                        + label_test
                        + "_snrmap.fits"
        ...
        if do_adi_full:
            ...
            if not fake_planet:
                ...
                for pp, npc in enumerate(test_pcs_adi_full):
                    for zz in range(nz):
                        params_pca = PCA_Params(
                            cube=PCA_ASDI_cube[zz],
                            angle_list=derot_angles,
                            cube_ref=ref_cube[zz],
                            scale_list=None,
                            ncomp=int(npc),
                            svd_mode=svd_mode,
                            scaling=scaling,
                    ...
                    if planet:
                        ...
                        plt.savefig(
                            ...
                            + "SNR_"
                            + source
                            + "_PCA-ADI-full_npc{:.0f}".format(npc)
                            + ".pdf",
                            format="pdf",
                        )
                        write_fits(
                            outpath
                            + "PCA-ADI_full_SNR_npc{:.0f}".format(npc)
                            + label_test
                            + ".fits",
                            snr_tmp,
                        )
                    write_fits(
                        outpath
                        + "PCA-ADI_full_npc{:.0f}".format(npc)
                        + label_test
                        + ".fits",
                        tmp[pp],
                    )  # all channels
                    tmp_tmp[pp] = np.median(
                        tmp[pp], axis=0
                    )  # axis zero because we sliced the pcs axis

                if planet:
                    opt_pp = np.argmax(snr_tmp, axis=0)
                    tmp_tmp_tmp = np.zeros(
                        [nz, PCA_ASDI_cube.shape[2], PCA_ASDI_cube.shape[3]]
                    )
                    for zz in range(nz):
                        tmp_tmp_tmp[zz] = tmp[opt_pp[zz], zz]
                    write_fits(
                        outpath
                        + "final_PCA-ADI_full_opt_npc_planet{}.fits".format(
                            label_test
                        ),
                        tmp_tmp_tmp,
                    )
                    ...
                    write_fits(
                        outpath
                        + "PCA-ADI_full_OPT_NPC_planet{}.fits".format(
                            label_test
                        ),
                        test_pcs_adi_full[opt_pp],
                    )
                    ...
                    plt.title(
                        ...
                        + " b"
                        + details
                        + "(PCA-ADI full - spectral channels collapsed)"
                    )
                    plt.ylabel("SNR")
                    plt.xlabel("npc")
                    for pp, npc in enumerate(test_pcs_adi_full):
                    ...
                    plt.savefig(
                        ...
                        + "SNR_"
                        + source
                        + "_PCA-ADI-full_collapsed.pdf",
                        format="pdf",
                    )
                    write_fits(
                        outpath
                        + "PCA-ADI_full_collapsed_SNR{}.fits".format(
                            label_test
                        ),
                        snr_tmp,
                    )
                    write_fits(
                        outpath
                        + "final_PCA-ADI_full_collapsed_opt_npc{:.0f}_planet{}.fits".format(
                            npc_opt, label_test
                        ),
                        tmp_tmp[opt_pp],
                    )
                write_fits(
                    outpath
                    + "PCA-ADI_full_collapsed_{}{}.fits".format(
                        test_pcs_str, label_test
                    ),
                    tmp_tmp,
                )
            else:
                ...
                for ns in range(nspi):
                    ...
                    for pp, npc in enumerate(test_pcs_adi_full):
                        ...
                        if debug:
                            write_fits(
                                outpath
                                + "TMP_PCA-ADI_full_npc{:.0f}{}_fcp_spi{:.0f}.fits".format(
                                    npc, label_test, ns
                                ),
                                tmp,
                            )
                ...
                for ff in range(nfcp):
                    ...
                    for pp, npc in enumerate(test_pcs_adi_full):
                        plt.plot(npc, snr_fcp[pp, ff], marker)
                plt.savefig(
                    outpath_fig + "SNR_fcps_PCA-ADI_full" + ".pdf",
                    format="pdf",
                )
                write_fits(
                    outpath
                    + "PCA-ADI_full_SNR_fcps_"
                    + test_pcs_str
                    + label_test
                    + ".fits",
                    snr_fcp,
                ...
                for pp, npc in enumerate(id_npc_adi_full_df):
                    ...
                    if debug:
                        write_fits(
                            outpath
                            + "TMP_PCA-ADI_full_opt_npc{:.0f}{}_fcp_spi{:.0f}.fits".format(
                                npc, label_test, ns
                            ),
                            tmp,
                        )
                ...
                write_fits(
                    outpath
                    + "final_PCA-ADI_full_image_{}_at_{}as".format(
                        test_pcs_str, test_rad_str
                    )
                    + label_test
                    + ".fits",
                ...
                write_fits(
                    outpath
                    + "final_PCA-ADI_full_opt_npc_at_{}as".format(test_rad_str)
                    + label_test
                    + ".fits",
                    id_npc_adi_full_df,
                )
                ...
                if (
                    not isfile(
                        outpath
                        + "final_PCA-ADI_full_snrmap_{}_at_{}as".format(
                            test_pcs_str, test_rad_str
                        )
                        + label_test
                        + ".fits"
        ...
        if do_adi_ann:
            ...
            if not fake_planet:
                    ...
                    if (
                        ...
                        if planet:
                            ...
                            plt.savefig(
                                ...
                                + "SNR_"
                                + source
                                + "_PCA-ADI-ann_npc{:.0f}".format(npc)
                                + ".pdf",
                                format="pdf",
                            )
                            write_fits(
                                outpath
                                + "PCA-ADI_ann_SNR_npc{:.0f}".format(npc)
                                + label_test_ann
                                + ".fits",
                                snr_tmp,
                            )
                ...
                if planet:
                    ...
                    write_fits(
                        outpath
                        + "final_PCA-ADI_ann_opt_npc_planet{}.fits".format(
                            label_test_ann
                        ),
                        tmp_tmp_tmp,
                    )
                    ...
                    write_fits(
                        outpath
                        + "PCA-ADI_ann_OPT_NPC_planet{}.fits".format(
                            label_test_ann
                        ),
                        opt_npc,
                    )
                    ...
                    plt.title(
                        ...
                        + " b"
                        + details
                        + "(PCA-ADI ann - specctral channels collapsed)"
                    )
                    plt.ylabel("SNR")
                    plt.xlabel("npc")
                    for pp, npc in enumerate(test_pcs_adi_ann):
                    ...
                    plt.savefig(
                        ...
                        + "SNR_"
                        + source
                        + "_PCA-ADI-ann_collapsed.pdf",
                        format="pdf",
                    )
                    write_fits(
                        outpath
                        + "PCA-ADI_ann_collapsed_SNR{}.fits".format(
                            label_test_ann
                        ),
                        snr_tmp,
                    )
                    write_fits(
                        outpath
                        + "final_PCA-ADI_ann_collapsed_opt_npc{:.0f}_planet{}.fits".format(
                            npc_opt, label_test_ann
                        ),
                        tmp_tmp[opt_pp],
                    )
                write_fits(
                    outpath
                    + "PCA-ADI_ann_collapsed_{}{}.fits".format(
                        test_pcs_str, label_test_ann
                    ),
                    tmp_tmp,
                )
            else:
                ...
                for ns in range(nspi):
                    ...
                    for pp, npc in enumerate(test_pcs_adi_ann):
                        ...
                        if debug:
                            write_fits(
                                outpath
                                + "TMP_PCA-ADI_ann_npc{:.0f}{}_fcp_spi{:.0f}.fits".format(
                                    npc, label_test_ann, ns
                                ),
                                tmp,
                            )
                ...
                for ff in range(nfcp):
                    ...
                    for pp, npc in enumerate(test_pcs_adi_ann):
                        plt.plot(npc, snr_fcp[pp, ff], marker)
                plt.savefig(
                    outpath_fig + "SNR_fcps_PCA-ADI_ann" + ".pdf", format="pdf"
                )
                write_fits(
                    outpath
                    + "PCA-ADI_ann_SNR_fcps_"
                    + test_pcs_str
                    + label_test_ann
                    + ".fits",
                    snr_fcp,
                ...
                for pp, npc in enumerate(id_npc_adi_ann_df):
                    ...
                    if debug:
                        write_fits(
                            outpath
                            + "TMP_PCA-ADI_ann_opt_npc{:.0f}{}_fcp_spi{:.0f}.fits".format(
                                npc, label_test_ann, ns
                            ),
                            tmp,
                        )
                ...
                write_fits(
                    outpath
                    + "final_PCA-ADI_ann_image_{}_at_{}as".format(
                        test_pcs_str, test_rad_str
                    )
                    + label_test_ann
                    + ".fits",
                ...
                write_fits(
                    outpath
                    + "final_PCA-ADI_ann_opt_npc_at_{}as".format(test_rad_str)
                    + label_test_ann
                    + ".fits",
                    id_npc_adi_ann_df,
                )
                ...
                if (
                    not isfile(
                        outpath
                        + "final_PCA-ADI_ann_snrmap_{}_at_{}as".format(
                            test_pcs_str, test_rad_str
                        )
                        + label_test_ann
                        + ".fits"
        ...
        if do_sadi_full and sadi_steps == 1:
            ...
            for ss, scal in enumerate(scalings):
                if scal is None:
                    ...
                    scal_lab = scal
                for ii, ifs_collapse_range in enumerate(
                    ifs_collapse_range_list
                ):
                    label_test = (
                        label_test_ori
                    ...
                    if not fake_planet:
                        ...
                        for pp, npc in enumerate(test_pcs_sadi_full):
                            ...
                            if debug:
                                write_fits(
                                    outpath
                                    + "TMP_final_PCA-SADI1_full_npc{:.0f}_".format(
                                        npc
                                    )
                                    + test_pcs_str
                                    + label_test
                        ...
                        write_fits(
                            outpath
                            + "final_PCA-SADI1_full_SNR_fcps_"
                            + test_pcs_str
                            + label_test
                            + ".fits",
                            snr_fcp,
                        ...
                        for ff in range(nfcp):
                            idx_best_snr = np.argmax(snr_fcp[:, ff])
                            id_npc_full_df[ff] = test_pcs_sadi_full[
                                idx_best_snr
                            ]
                        ...
                        test_pcs_str_list = [
                            str(int(x)) for x in id_npc_full_df
                        ]
                        test_pcs_str = "npc_opt" + "-".join(test_pcs_str_list)
                        test_rad_str_list = [
                            "{:.1f}".format(x) for x in rad_arr * plsc
                        ...
                        write_fits(
                            outpath
                            + "final_PCA-SADI1_full_image_{}_at_{}as".format(
                                test_pcs_str, test_rad_str
                            )
                            + label_test
                            + ".fits",
                        ...
                        write_fits(
                            outpath
                            + "final_PCA-SADI1_full_opt_npc_at_{}as".format(
                                test_rad_str
                            )
                            + label_test
                            + ".fits",
                        ...
                        if (
                            not isfile(
                                outpath
                                + "final_PCA-SADI1_full_snrmap_{}_at_{}as".format(
                                    test_pcs_str, test_rad_str
                                )
                                + label_test
                                + ".fits"
                    ...
                    if not fake_planet:
                        ...
                        for pp1, npc1 in enumerate(test_pcs_sadi_full_sdi):
                            for pp2, npc2 in enumerate(test_pcs_sadi_full_adi):
                                ...
                                tmp[counter], tmp_tmp, tmp_tmp_der = pca(
                                    algo_params=params_pca
                                )
                                if do_stim_map:
                                    stim_map[counter] = compute_stim_map(
                                        tmp_tmp_der
                                    )
                                    inv_stim_map[counter] = (
                        ...
                        if planet:
                            ...
                            counter = 0
                            for pp1, npc1 in enumerate(test_pcs_sadi_full_sdi):
                                for pp2, npc2 in enumerate(
                                    test_pcs_sadi_full_adi
                                ):
                        ...
                        plt.figure()
                        plt.title(
                            "SNR for fcps " + details + "(PCA-SADI2 full)"
                        )
                        plt.ylabel("SNR")
                        plt.xlabel("npc")
                        for ff in range(nfcp):
                            ...
                            for pp1, npc1 in enumerate(test_pcs_sadi_full_sdi):
                                for pp2, npc2 in enumerate(
                                    ...
                                    if pp2 == 0:
                                        label = "fcp {:.0f} - npc sdi = {:.0f}".format(
                                            ff + 1, npc1
                                        )
                                    else:
                                        label = None
                                    plt.plot(
                                        npc2,
                                        snr_fcp[counter, ff],
                                        colors[pp1 % len(colors)] + marker,
                                        label=label,
                                    )
                                    counter += 1
                        plt.savefig(
                            outpath_fig + "SNR_fcps_PCA-SADI2-full" + ".pdf",
                            format="pdf",
                        )

                        write_fits(
                            outpath
                            + "final_PCA-SADI2_full_SNR_fcps_"
                            + test_pcs_str
                            + label_test
                            + ".fits",
                            snr_fcp,
                        ...
                        for ff in range(nfcp):
                            idx_best_snr = np.argmax(snr_fcp[:, ff])
                            id_npc_full_df[ff, 0] = test_pcs_sadi_full_sdi[
                                int(idx_best_snr / ntest_pcs_adi)
                            ]
                            id_npc_full_df[ff, 1] = test_pcs_sadi_full_adi[
                                int(idx_best_snr % ntest_pcs_adi)
                            ]
                        ...
                        test_pcs_str_list_sdi = [
                            str(x) for x in test_pcs_sadi_full_sdi
                        ]
                        ntest_pcs_sdi = len(test_pcs_sadi_full_sdi)
                        test_pcs_str_sdi = "npc_sdi" + "-".join(
                            test_pcs_str_list_sdi
                        )
                        test_pcs_str_list_adi = [
                            str(x) for x in test_pcs_sadi_full_adi
                        ]
                        ntest_pcs_adi = len(test_pcs_sadi_full_adi)
                        test_pcs_str_adi = "_npc_adi" + "-".join(
                            test_pcs_str_list_adi
                        )
                        ntest_pcs = int(ntest_pcs_sdi * ntest_pcs_adi)
                        test_pcs_str = test_pcs_str_sdi + test_pcs_str_adi
                        ...
                        test_pcs_str_list_sdi = [
                            str(int(x)) for x in id_npc_full_df[:, 0]
                        ]
                        test_pcs_str_sdi = "npc_sdi_opt" + "-".join(
                            test_pcs_str_list_sdi
                        )
                        test_pcs_str_list_adi = [
                            str(int(x)) for x in id_npc_full_df[:, 1]
                        ]
                        test_pcs_str_adi = "npc_adi_opt" + "-".join(
                            test_pcs_str_list_adi
                        )
                        ...
                        write_fits(
                            outpath
                            + "final_PCA-SADI2_full_image_{}_at_{}as".format(
                                test_pcs_str, test_rad_str
                            )
                            + label_test
                            + ".fits",
                        ...
                        write_fits(
                            outpath
                            + "final_PCA-SADI2_full_opt_npc_at_{}as".format(
                                test_rad_str
                            )
                            + label_test
                            + ".fits",
                        ...
                        if (
                            not isfile(
                                outpath
                                + "final_PCA-SADI2_full_snrmap_{}_at_{}as".format(
                                    test_pcs_str, test_rad_str
                                )
                                + label_test
                                + ".fits"
                            ...
                            tmp = open_fits(
                                outpath
                                + "final_PCA-SADI2_image_full_{}_at_{}as".format(
                                    test_pcs_str, test_rad_str
                                )
                                + label_test
                                + ".fits"
                    ...
                    if not fake_planet:
                        ...
                        for pp1, npc1 in enumerate(test_pcs_sadi_ann_sdi):
                            for pp2, npc2 in enumerate(test_pcs_sadi_ann_adi):
                                ...
                                tmp_tmp, tmp_tmp_der, tmp[counter] = (
                                    pca_annular(algo_params=params_ann)
                                )
                                if do_stim_map:
                                    stim_map[counter] = compute_stim_map(
                                        tmp_tmp_der
                        ...
                        for ns in range(nspi):
                            ...
                            for pp1, npc1 in enumerate(test_pcs_sadi_ann_sdi):
                                for pp2, npc2 in enumerate(
                                    ...
                                    tmp_tmp[counter] = pca_annular(
                                        algo_params=params_ann
                                    )
                                    for ff in range(nfcp):
                                        xx_fcp = cx + rad_arr[ff] * np.cos(
                                            np.deg2rad(theta0 + ff * th_step)
                        ...
                        for ff in range(nfcp):
                            ...
                            for pp1, npc1 in enumerate(test_pcs_sadi_ann_sdi):
                                for pp2, npc2 in enumerate(
                                    ...
                                    counter += 1
                        plt.savefig(
                            outpath_fig + "SNR_fcps_PCA-SADI2-ann" + ".pdf",
                            format="pdf",
                        )

                        write_fits(
                            outpath
                            + "final_PCA-SADI2_ann_SNR_fcps_"
                            + test_pcs_str
                            + label_test_ann
                            + ".fits",
                            snr_fcp,
                        ...
                        for ff in range(nfcp):
                            idx_best_snr = np.argmax(snr_fcp[:, ff])
                            id_npc_ann_df[ff, 0] = test_pcs_sadi_ann_sdi[
                                int(idx_best_snr / ntest_pcs_adi)
                            ]
                            id_npc_ann_df[ff, 1] = test_pcs_sadi_ann_adi[
                                int(idx_best_snr % ntest_pcs_adi)
                            ]
                        ...
                        test_pcs_str_list_sdi = [
                            str(x) for x in test_pcs_sadi_ann_sdi
                        ]
                        ntest_pcs_sdi = len(test_pcs_sadi_ann_sdi)
                        test_pcs_str_sdi = "npc_sdi" + "-".join(
                            test_pcs_str_list_sdi
                        ...
                        test_pcs_str_list_adi = [
                            str(x) for x in test_pcs_sadi_ann_adi
                        ]
                        ntest_pcs_adi = len(test_pcs_sadi_ann_adi)
                        test_pcs_str_adi = "_npc_adi" + "-".join(
                            test_pcs_str_list_adi
                        ...
                        test_pcs_str_list_sdi = [
                            str(int(x)) for x in id_npc_ann_df[:, 0]
                        ]
                        test_pcs_str_sdi = "npc_sdi_opt" + "-".join(
                            test_pcs_str_list_sdi
                        )
                        test_pcs_str_list_adi = [
                            str(int(x)) for x in id_npc_ann_df[:, 1]
                        ]
                        test_pcs_str_adi = "npc_adi_opt" + "-".join(
                            test_pcs_str_list_adi
                        )
                        ...
                        write_fits(
                            outpath
                            + "final_PCA-SADI2_ann_image_{}_at_{}as".format(
                                test_pcs_str, test_rad_str
                            )
                            + label_test_ann
                            + ".fits",
                        ...
                        write_fits(
                            outpath
                            + "final_PCA-SADI2_ann_opt_npc_at_{}as".format(
                                test_rad_str
                            )
                            + label_test_ann
                            + ".fits",
                        ...
                        if (
                            not isfile(
                                outpath
                                + "final_PCA-SADI2_ann_snrmap_{}_at_{}as".format(
                                    test_pcs_str, test_rad_str
                                )
                                + label_test_ann
                                + ".fits"
            ...
            for ii, ifs_collapse_range in enumerate(ifs_collapse_range_list):
                ...
                if do_sdi and fake_planet:
                    ...
                    for rr, rad in enumerate(rad_arr):
                        if svd_mode == "randsvd":
                            for nr in range(3):
                                pn_contr_curve_sdi_rr_tmp = contrast_curve(
                                    PCA_ASDI_cube_ori,
                                    derot_angles,
                                    psfn,
                                    fwhm,
                                    ...
                                    adimsdi="double",
                                    verbose=verbose,
                                    ncomp=(int(id_npc_full_df[rr]), None),
                                    svd_mode=svd_mode,
                                    nproc=nproc,
                                    imlib=imlib,
                                )
                                rsvd_list.append(pn_contr_curve_sdi_rr_tmp)
                            pn_contr_curve_sdi_rr = (
                                pn_contr_curve_sdi_rr_tmp.copy()
                            )
                            for jj in range(pn_contr_curve_full_rr.shape[0]):
                                sensitivities = []
                                for nr in range(3):
                                    sensitivities.append(
                                        rsvd_list[nr]["sensitivity (Student)"][
                                            jj
                                        ]
                                    )
                                print(
                                    "Sensitivities at {}: ".format(
                                        rsvd_list[rr]["distance"][jj]
                                    ),
                                    sensitivities,
                                )
                                idx_min = np.argmin(sensitivities)
                                pn_contr_curve_sdi_rr["sensitivity (Student)"][
                                    jj
                                ] = rsvd_list[idx_min][
                                    "sensitivity (Student)"
                                ][
                                ...
                                pn_contr_curve_sdi_rr["sensitivity (Gauss)"][
                                    jj
                                ] = rsvd_list[idx_min]["sensitivity (Gauss)"][
                                    jj
                                ]
                                pn_contr_curve_sdi_rr["throughput"][jj] = (
                                    rsvd_list[idx_min]["throughput"][jj]
                                )
                                pn_contr_curve_sdi_rr["noise"][jj] = rsvd_list[
                                    idx_min
                                ]["noise"][jj]
                                pn_contr_curve_sdi_rr["sigma corr"][jj] = (
                                    rsvd_list[idx_min]["sigma corr"][jj]
                                )
                        else:
                            pn_contr_curve_sdi_rr = contrast_curve(
                                PCA_ASDI_cube_ori,
                                derot_angles,
                                psfn,
                                fwhm,
                        ...
                        DF.to_csv(
                            pn_contr_curve_sdi_rr,
                            path_or_buf=outpath
                            + "contrast_curve_PCA-SDI_optimal_at_{:.1f}as.csv".format(
                                rad * plsc
                            ),
                            sep=",",
                            na_rep="",
                    ...
                    for jj in range(pn_contr_curve_sdi_opt.shape[0]):
                        ...
                        idx_min = np.argmin(sensitivities)
                        pn_contr_curve_sdi_opt["sensitivity (Student)"][jj] = (
                            df_list[idx_min]["sensitivity (Student)"][jj]
                        )
                        pn_contr_curve_sdi_opt["sensitivity (Gauss)"][jj] = (
                            df_list[idx_min]["sensitivity (Gauss)"][jj]
                        )
                        pn_contr_curve_sdi_opt["throughput"][jj] = df_list[
                            idx_min
                        ]["throughput"][jj]
                        pn_contr_curve_sdi_opt["noise"][jj] = df_list[idx_min][
                            "noise"
                        ][jj]
                        pn_contr_curve_sdi_opt["sigma corr"][jj] = df_list[
                            idx_min
                        ]["sigma corr"][jj]
                    DF.to_csv(
                        pn_contr_curve_sdi_opt,
                        path_or_buf=outpath
                        + "final_optimal_contrast_curve_PCA-SDI"
                        + label_test
                        + ".csv",
                        sep=",",
                        na_rep="",
                    ...
                    arr_contrast = np.array(
                        pn_contr_curve_sdi_opt["sensitivity (Student)"]
                    )
                    for ff in range(nfcp):
                        idx = find_nearest(arr_dist, rad_arr[ff])
                ...
                if do_sadi_full and fake_planet:
                    ...
                    if sadi_steps == 1:
                        ...
                        adimsdi = "double"
                        npc_opt = (
                            int(id_npc_full_df[rr, 0]),
                            int(id_npc_full_df[rr, 1]),
                        )
                    for rr, rad in enumerate(rad_arr):
                        pn_contr_curve_full_rr = contrast_curve(
                            PCA_ASDI_cube_ori,
                        ...
                        DF.to_csv(
                            pn_contr_curve_full_rr,
                            path_or_buf=outpath
                            + "contrast_curve_PCA-SADI{:.0f}-full_optimal_at_{:.1f}as.csv".format(
                                sadi_steps, rad * plsc
                            ),
                            sep=",",
                            na_rep="",
                    ...
                    for jj in range(pn_contr_curve_full_opt.shape[0]):
                        ...
                        idx_min = np.argmin(sensitivities)
                        pn_contr_curve_full_opt["sensitivity (Student)"][
                            jj
                        ] = df_list[idx_min]["sensitivity (Student)"][jj]
                        pn_contr_curve_full_opt["sensitivity (Gauss)"][jj] = (
                            df_list[idx_min]["sensitivity (Gauss)"][jj]
                        )
                        pn_contr_curve_full_opt["throughput"][jj] = df_list[
                            idx_min
                        ]["throughput"][jj]
                        pn_contr_curve_full_opt["noise"][jj] = df_list[
                            idx_min
                        ]["noise"][jj]
                        pn_contr_curve_full_opt["sigma corr"][jj] = df_list[
                            idx_min
                        ]["sigma corr"][jj]
                    DF.to_csv(
                        pn_contr_curve_full_opt,
                        path_or_buf=outpath
                        + "final_optimal_contrast_curve_PCA-SADI{:.0f}-full{}.csv".format(
                            sadi_steps, label_test
                        ),
                        sep=",",
                        na_rep="",
                        float_format=None,
                ...
                if do_sadi_ann and fake_planet:
                    ...
                    df_list = []
                    for rr, rad in enumerate(rad_arr):
                        pn_contr_curve_ann_rr = contrast_curve(
                            PCA_ASDI_cube_ori,
                            derot_angles,
                            psfn,
                            fwhm,
                            ...
                            verbose=verbose,
                            ncomp=(
                                int(id_npc_ann_df[rr, 0]),
                                int(id_npc_ann_df[rr, 1]),
                            ),
                            svd_mode=svd_mode,
                            radius_int=mask_IWA_px,
                            asize=asize,
                        ...
                        DF.to_csv(
                            pn_contr_curve_ann_rr,
                            path_or_buf=outpath
                            + "contrast_curve_PCA-SADI2-ann_optimal_at_{:.1f}as.csv".format(
                                rad * plsc
                            ),
                            sep=",",
                            na_rep="",
                    ...
                    for jj in range(pn_contr_curve_ann_opt.shape[0]):
                        ...
                        idx_min = np.argmin(sensitivities)
                        pn_contr_curve_ann_opt["sensitivity (Student)"][jj] = (
                            df_list[idx_min]["sensitivity (Student)"][jj]
                        )
                        pn_contr_curve_ann_opt["sensitivity (Gauss)"][jj] = (
                            df_list[idx_min]["sensitivity (Gauss)"][jj]
                        )
                        pn_contr_curve_ann_opt["throughput"][jj] = df_list[
                            idx_min
                        ]["throughput"][jj]
                        pn_contr_curve_ann_opt["noise"][jj] = df_list[idx_min][
                            "noise"
                        ][jj]
                        pn_contr_curve_ann_opt["sigma corr"][jj] = df_list[
                            idx_min
                        ]["sigma corr"][jj]
                    DF.to_csv(
                        pn_contr_curve_ann_opt,
                        path_or_buf=outpath
                        + "final_optimal_contrast_curve_PCA-SADI2-ann.csv",
                        sep=",",
                        na_rep="",
                        float_format=None,
                    )
                    ...
                    arr_contrast = np.array(
                        pn_contr_curve_ann_opt["sensitivity (Student)"]
                    )
                    for ff in range(nfcp):
                        idx = find_nearest(arr_dist, rad_arr[ff])
                ...
                if fake_planet:
                    ...
                    plt.figure()
                    plt.title(
                        "5-sigma contrast curve for " + source + " (NACO+AGPM)"
                    )
                    plt.ylabel("Contrast")
                    plt.xlabel("Separation (arcsec)")
                    if do_sdi:
                        plt.semilogy(
                            pn_contr_curve_sdi_opt["distance"] * plsc,
                            pn_contr_curve_sdi_opt["sensitivity (Student)"],
                            "r",
                            linewidth=2,
                            label="PCA-SDI (Student correction)",
                        )
                    if do_sadi_full and fake_planet:
                        plt.semilogy(
                            pn_contr_curve_full_opt["distance"] * plsc,
                            pn_contr_curve_full_opt["sensitivity (Student)"],
                            "b",
                            linewidth=2,
                            label="PCA-SADI{:.0f} full frame (Student, lapack)".format(
                                sadi_steps
                            ),
                        )
                    if do_sadi_ann and fake_planet:
                        plt.semilogy(
                            pn_contr_curve_ann_opt["distance"] * plsc,
                            pn_contr_curve_ann_opt["sensitivity (Student)"],
                            "g",
                            linewidth=2,
                            label="PCA-SADI annular (Student)",
                        )
                    plt.legend()
                    plt.savefig(
                        outpath_fig + "contr_curves" + ".pdf", format="pdf"
                    )
                    ...
                    if do_sadi_ann and fake_planet:
                        plt.plot(
                            ...
                            * np.log10(
                            ...
                            "g",
                            linewidth=2,
                            label="PCA-SADI2 annular (Student)",
                        )
                    plt.legend()
                    plt.savefig(
                        outpath_fig + "contr_curves_MAG" + ".pdf", format="pdf"
                    )

                    # WRITE THE CSV FILE
                    datafr1 = DF(
                        data=nfcp_df, columns=["Index of injected fcp"]
                    )
                    ...
                    if do_sdi:
                        datafr8 = DF(
                            data=sensitivity_5sig_sdi_df,
                            columns=["5-sig Student sensitivity (PCA-SDI)"],
                        )
                    ...
                    if do_sadi_full:
                        datafr10 = DF(
                            data=id_npc_full_df,
                            columns=[
                                "Ideal npc (PCA-SADI{:.0f}-full)".format(
                                    sadi_steps
                                )
                            ],
                        )
                        datafr12 = DF(
                            data=sensitivity_5sig_full_df,
                            columns=[
                                "5-sig Student sensitivity (PCA-SADI{:.0f}-full, lapack)".format(
                                    sadi_steps
                                )
                            ],
                        )
                    ...
                    if do_sadi_ann and fake_planet:
                        datafr14 = DF(
                            data=id_npc_ann_df,
                            columns=["Ideal npc (PCA-SADI2 ann)"],
                        )
                        datafr16 = DF(
                            data=sensitivity_5sig_ann_df,
                            columns=[
                                "5-sig Student sensitivity (PCA-SADI2-ann)"
                            ],
                        )
                    ...
                    DF.to_csv(
                        datafr,
                        path_or_buf=outpath
                        + "Final_contrast_curves_comparison"
                        + label_test
                        + ".csv",
                        sep=",",
                        na_rep="",
