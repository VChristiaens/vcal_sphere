                        ...
                        if svd_mode == 'randsvd':
                            ...
                            for nr in range(3):
                                params_pca = PCAParams(cube=PCA_ASDI_cube, angle_list=derot_angles, cube_ref=None,
                                                       ...
                                                       verbose=verbose, conv=do_conv, nproc=nproc)
                ...
                for pp, npc in enumerate(id_npc_sdi_df):
                    if svd_mode == 'randsvd':
                        ...
                        params_pca = PCAParams(cube=PCA_ASDI_cube_ori, angle_list=derot_angles, cube_ref=None,
                                               ...
                                               adimsdi='double', full_output=False, verbose=verbose, conv=do_conv,
                                               nproc=nproc)
                        tmp_tmp[pp] = pca(algo_params=params_pca)
                                ...
                                if svd_mode == 'randsvd':
                                    ...
                                    for nr in range(3):
                                        params_pca = PCAParams(cube=PCA_ASDI_cube, angle_list=derot_angles,
                                                               ...
                                                               verbose=verbose, conv=do_conv, mask_rdi=mask_rdi,
                                                               nproc=nproc)
                                        tmp_tmp_tmp[nr] = pca(algo_params=params_pca)
                                    tmp_tmp[pp] = np.median(tmp_tmp_tmp, axis=0)
                                else:
                        ...
                        for pp1, npc1 in enumerate(test_pcs_sadi_full_sdi):
                            for pp2, npc2 in enumerate(test_pcs_sadi_full_adi):
                                params_pca = PCAParams(cube=PCA_ASDI_cube, angle_list=derot_angles, cube_ref=None,
                                                       ...
                                                       check_memory=True, ifs_collapse_range=ifs_collapse_range,
                                                       full_output=True, verbose=verbose, conv=do_conv,
                                                       mask_rdi=mask_rdi, nproc=nproc)
