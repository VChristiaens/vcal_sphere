...
def calib(params_calib_name='VCAL_params_calib.json'):
    ...
    *******
    ...
    with open(vcal_path[0] + "/instr_param/sphere_filt_spec.json", 'r') as filt_spec_file:
        ...
        filt_spec = json.load(filt_spec_file)[comb_iflt]
    ...
    poly_order_wc = params_calib.get('poly_order_wc', 1)
    ...
    #                vip_hci.fits.write_fits("{}master_masks_for_PCA_sky_psf.fits".format(outpath_irdis_fits), mask_arr)
        ...
        if 6 in to_do:
            ...
            if science_mode == 'DBI':
                ...
                raise ValueError(
                    "science_mode not recognized: should be DBI or CI.")
                ...
                for ii in range(len(sci_list_irdis)):
                    if not isfile(outpath_irdis_sof+"OBJECT{:.0f}.sof".format(ii)) or overwrite_sof:
                        with open(outpath_irdis_sof+"OBJECT{:.0f}.sof".format(ii), 'w') as f:
                            f.write(inpath+label_ss +
                                    sci_list_irdis[ii]+'\t'+lab_SCI)
                            if pca_subtr:
                                f.write("{}{}master_dark.fits".format(
                            ...
                            if isfile(join(inpath_filt_table, "sph_ird_filt_table.fits")):
                                f.write(
                                    join(inpath_filt_table, "sph_ird_filt_table.fits")+'\t'+'IRD_FILTER_TABLE\n')
                            f.write("{}FINAL_badpixelmap.fits".format(
                                outpath_irdis_fits)+'\t'+'IRD_STATIC_BADPIXELMAP')
                    ...
                    if not isfile(outpath_irdis_fits+"science_dbi{:.0f}.fits".format(ii)) or overwrite_sof or overwrite_fits:
                        ...
                        command += " --ird.{}.outfilename_left={}science_{}{:.0f}.fits".format(
                            lab_rec, outpath_irdis_fits, lab_lr[0], ii)
                        command += " --ird.{}.outfilename_right={}science_{}{:.0f}.fits".format(
                            lab_rec, outpath_irdis_fits, lab_lr[1], ii)
                ...
                for ii in range(len(cen_list_irdis)):
                    if not isfile(outpath_irdis_sof+"CEN{:.0f}.sof".format(ii)) or overwrite_sof:
                        with open(outpath_irdis_sof+"CEN{:.0f}.sof".format(ii), 'w') as f:
                            ...
                            if isfile(inpath_filt_table+"sph_ird_filt_table.fits"):
                                f.write(
                                    inpath_filt_table+"sph_ird_filt_table.fits"+'\t'+'IRD_FILTER_TABLE\n')
                            f.write("{}FINAL_badpixelmap.fits".format(
                                outpath_irdis_fits)+'\t'+'IRD_STATIC_BADPIXELMAP')
                    ...
                    if not isfile(outpath_irdis_fits+"cen_dbi{:.0f}.fits".format(ii)) or overwrite_sof or overwrite_fits:
                        ...
                        command += " --ird.{}.outfilename_left={}cen_{}{:.0f}.fits".format(
                            lab_rec, outpath_irdis_fits, lab_lr[0], ii)
                        command += " --ird.{}.outfilename_right={}cen_{}{:.0f}.fits".format(
                            lab_rec, outpath_irdis_fits, lab_lr[1], ii)
    #                    if crop_sz_irdis>0 and crop_sz_irdis<1024:
    #                        command+= " --ird.{}.window_size={:.0f}".format(lab_rec,crop_sz_irdis)
                ...
                for ii in range(len(psf_list_irdis)):
                    ...
                    if not isfile(outpath_irdis_fits+"psf_dbi{:.0f}.fits".format(ii)) or overwrite_sof or overwrite_fits:
                        ...
                        command += " --ird.{}.outfilename_left={}psf_{}{:.0f}.fits".format(
                            lab_rec, outpath_irdis_fits, lab_lr[0], ii)
                        command += " --ird.{}.outfilename_right={}psf_{}{:.0f}.fits".format(
                            lab_rec, outpath_irdis_fits, lab_lr[1], ii)
    ...
    #                        command+= " --ird.{}.window_size={:.0f}".format(lab_rec,crop_sz_psf_irdis)
            ...
            if not pca_subtr or not pca_subtr_psf:
                ...
                lab_ird = ["_left", "_right"]
                file_list = os.listdir(curr_path)
                for rr in range(len(lab_lr)):
                    products_list = [x for x in file_list if (
                        x.startswith(instr) and x.endswith(lab_ird[rr]+".fits"))]
                    for pp, prod in enumerate(products_list):
                        ...
                        for zz in range(tmp.shape[0]):
                            ...
                            tmp[zz] = tmp[zz] - avg
                        hdul[0].data = tmp
                        hdul.writeto(prod, output_verify='ignore',
                                     overwrite=True)
        ...
        if 10 in to_do or 15 in to_do or 17 in to_do:
            ...
            if len(dark_list_ifs) < 1:
                ...
                dark_cube, dark_head = open_fits(
                    inpath+dark_list_ifs[0], header=True)
            # OBJECT
            ...
            counter = 0
            #nmd_fr = int(nd_fr*nd)
            # ] np.zeros([nmd_fr,dark_cube.shape[1],dark_cube.shape[2]])
            master_dark_cube = []
            with open(outpath_ifs_sof+"master_dark.sof", 'w+') as f:
                for ii in range(len(dark_list_ifs)):
                    dark_cube, dark_head = open_fits(
                        inpath+dark_list_ifs[ii], header=True)
            ...
            if indiv_fdark and len(dico_lists['dit_ifs_flat']) > 1:
                ...
                for nn, fdit in enumerate(dit_ifs_flat_list):
                    ...
                    dark_cube = open_fits(
                        inpath+fdark_list_ifs[0], header=False)
                    nd_fr = dark_cube.shape[0]
                    counter = 0
                    ...
                    with open(outpath_ifs_sof+"master_fdark{:.0f}.sof".format(nn), 'w+') as f:
                        for dd in range(nfd):
                            dark_cube, fdark_head = open_fits(
                                inpath+fdark_list_ifs[dd], header=True)
                            dit_fdark = fdark_head['HIERARCH ESO DET SEQ1 DIT']
                            if dit_fdark == fdit:
                                f.write(
                                    inpath+fdark_list_ifs[dd]+'\t'+'IFS_DARK_RAW\n')
                                master_dark_cube[counter:counter +
                                                 dark_cube.shape[0]] = dark_cube
                                counter += dark_cube.shape[0]
                    write_fits(
                    ...
                    master_dark_tmp = open_fits(
                        "{}master_dark{:.0f}.fits".format(outpath_ifs_fits, nn))
                    for nf in range(len(all_flat_lists_ifs)):
                        for ff, ff_name in enumerate(all_flat_lists_ifs[nf]):
                            ...
                            if dit_flat == fdit:
                                ...
                                # , output_verify)
                                hdulist.writeto(
                                    inpath+label_ds+ff_name, output_verify='ignore', overwrite=True)
            ...
            if indiv_fdark:
                ...
                os.system("rsync -va {}master_badpixelmap{:.0f}.fits {}master_badpixelmap.fits".format(
                    outpath_ifs_fits, len(dit_ifs_flat_list)-1, outpath_ifs_fits))
            ...
            with open(outpath_ifs_sof+"preamp.sof", 'w+') as f:
                ...
                for ii in range(len(flat_list_ifs_det_BB)):
                    tmp, header = open_fits(
                        inpath+flat_list_ifs_det_BB[ii], header=True)
                    f.write(inpath+lab_flat+label_ds +
                            flat_list_ifs_det_BB[ii]+'\t'+'IFS_DETECTOR_FLAT_FIELD_RAW\n')
            ...
            if large_scale_flat:
                ...
                for kk in range(1, 5):
                    with open(outpath_ifs_sof+"large_scale_flat_l{:.0f}.sof".format(kk), 'w+') as f:
                        ...
                        for ii in range(len(flat_list_ifs_det)):
                            tmp, header = open_fits(
                                inpath+flat_list_ifs_det[ii], header=True)
                            if 'HL{:.0f}'.format(kk) in header['HIERARCH ESO INS2 CAL']:
                                f.write(
                                    inpath+lab_flat+label_ds+flat_list_ifs_det[ii]+'\t'+'IFS_DETECTOR_FLAT_FIELD_RAW\n')
                                counter += 1
                ...
                if not isfile(outpath_ifs_sof+"large_scale_flat_BB.sof") or overwrite_sof:
                    ...
                    with open(outpath_ifs_sof+"large_scale_flat_BB.sof", 'w+') as f:
                        for ii in range(len(flat_list_ifs_det_BB)):
                            f.write(
                                inpath+lab_flat+label_ds+flat_list_ifs_det_BB[ii]+'\t'+'IFS_DETECTOR_FLAT_FIELD_RAW\n')
                        if ('FLAT' in dark_ifs and not indiv_fdark) or indiv_fdark:
                            f.write("{}master_dark.fits".format(
                                outpath_ifs_fits+label_fd)+'\t'+'IFS_MASTER_DARK\n')
                        f.write("{}master_badpixelmap.fits".format(
        ...
        if 13 in to_do:
            ...
            if not isfile(outpath_ifs_sof+"spectra_pos.sof") or overwrite_sof:
                ...
                with open(outpath_ifs_sof+"spectra_pos.sof", 'w+') as f:
                    ...
                    if indiv_fdark:
                        tmp, header = open_fits(
                            inpath+specpos_IFS[ii], header=True)
        ...
        if 15 in to_do:
                ...
                for ii in range(len(wave_calib_list_ifs)):
                    ...
                    for nn, fdit in enumerate(dit_ifs_flat_list):
                        ...
                        master_dark_cube = open_fits(
                            outpath_ifs_fits+"master_dark_cube{:.0f}.fits".format(nn))
                        if fdit == dit_wc:
                    ...
                    hdul.writeto(
                        inpath+skysub_lab_IFS+wave_calib_list_ifs[ii], output_verify='ignore', overwrite=True)
                    ...
                    if not isdir(inpath+lab_wc):
                        os.makedirs(inpath+lab_wc)
                    hdul.writeto(inpath+bpcorr_lab_IFS+'1_' +
                                 wave_calib_list_ifs[ii], output_verify='ignore', overwrite=True)
                    ...
                    if not isdir(inpath+lab_wc):
                        os.makedirs(inpath+lab_wc)
                    hdul.writeto(
                        inpath+bpcorr_lab_IFS+wave_calib_list_ifs[ii], output_verify='ignore', overwrite=True)
                    ...
                    if xtalk_corr:  # too agressive
                        ...
                        hdul[0].data = cube
                        hdul.writeto(
                            inpath+xtalkcorr_lab_IFS+wave_calib_list_ifs[ii], output_verify='ignore', overwrite=True)
                    # pdb.set_trace()
            ...
            if not isfile(outpath_ifs_fits+"wave_calib.fits") or overwrite_sof or overwrite_fits:
                ...
                command += "ifs.wave_calib.polyfit_order={:.0f}".format(
                    poly_order_wc)
                command += " --ifs.wave_calib.outfilename={}wave_calib.fits".format(
                    outpath_ifs_fits)
...
#                                lab = "{:.0f}".format(jj)
        ...
        if 17 in to_do and sky:
            ...
            if len(psf_sky_list_ifs) < 1:
                ...
                master_psf_sky = master_sky.copy()
...
#                        f.write("{}master_badpixelmap.fits".format(outpath_ifs_fits)+'\t'+'IFS_STATIC_BADPIXELMAP\n')
                ...
                if manual_sky:  # subtract residual sky level
                    ...
                    file_list = os.listdir(curr_path)
                    products_list = [x for x in file_list if (
                        x.startswith(lab_bp+instr) and x.endswith(".fits"))]
                    for pp, prod in enumerate(products_list):
                        hdul = fits.open(prod)
                        tmp = hdul[0].data
                        for zz in range(tmp.shape[0]):
                            ...
                            for aa, ap in enumerate(corner_coords):
                                ...
                                flux_tmp = photutils.aperture_photometry(
...
#                    fits.writeto(inpath+'skysub_lab_IFS'+lab_bp+sci_list_ifs[ii], tmp, head, output_verify='ignore', overwrite=True)
                ...
                if not isfile(outpath_ifs_sof+"CEN{}{:.0f}.sof".format(lab_distort, ii)) or overwrite_sof:
                    with open(outpath_ifs_sof+"CEN{}{:.0f}.sof".format(lab_distort, ii), 'w') as f:
                        f.write(inpath+lab_sci+lab_bp +
                                cen_list_ifs[ii]+'\t'+'IFS_SCIENCE_DR_RAW\n')
                        for jj in range(1, 6):
                            if not large_scale_flat or (jj == 5 and large_scale_flat == 'some'):
                                lab_sof = "{}master_flat_det_l{:.0f}.fits".format(
                                    outpath_ifs_fits, jj)  # v5c
        ...
        if 20 in to_do:
            ...
            for ii in range(len(psf_list_ifs)):
                ...
                if sky:
                    ...
                    hdul[0].data = cube
                    hdul.writeto(inpath+skysub_lab_IFS+lab_bp +
                                 psf_list_ifs[ii], output_verify='ignore', overwrite=True)
# ADAPT BELOW IF NECESSARY
#                if bpix:
#                    tmp = vip_hci.preproc.cube_fix_badpix_clump(tmp, bpm_mask=None, cy=None, cx=None, fwhm=4.,
#                          sig=4., protect_psf=True, verbose=True,
...
#                    fits.writeto(inpath+'skysub_lab_IFS'+lab_bp+sci_list_ifs[ii], tmp, head, output_verify='ignore', overwrite=True)
                ...
                if not isfile(outpath_ifs_sof+"PSF{}{:.0f}.sof".format(lab_distort, ii)) or overwrite_sof:
                    with open(outpath_ifs_sof+"PSF{}{:.0f}.sof".format(lab_distort, ii), 'w') as f:
                        f.write(inpath+skysub_lab_IFS+lab_bp +
                                psf_list_ifs[ii]+'\t'+'IFS_SCIENCE_DR_RAW\n')
                        for jj in range(1, 6):
                            if not large_scale_flat or (jj == 5 and large_scale_flat == 'some'):
                                lab_sof = "{}master_flat_det_l{:.0f}.fits".format(
                                    outpath_ifs_fits, jj)  # v5c
