...
def calib(params_calib_name="VCAL_params_calib.json") -> None:
    ...
    *******
    ...
    inpath = path + "raw/"
    if local_path:
        inpath_static = join(vcal_path, "Static/")
        f1 = inpath_static + "sph_ird_filt_table.fits"
        f2 = inpath_static + "ifs_lenslet_model_Y_J.txt"
        f3 = inpath_static + "sphere_filt_spec.json"
    else:
        try:
            f1 = download_file(
                join(url_d, "sph_ird_filt_table.fits"), cache=True
            )
            f2 = download_file(
                join(url_d, "ifs_lenslet_model_Y_J.txt"), cache=True
            )
            f3 = download_file(
    ...
    with open(f3, "r") as filt_spec_file:
        filt_spec = load(filt_spec_file)[comb_iflt]  # Get info on filter
    ...
    mask_pca_sky_sub = params_calib.get(
        "mask_pca_sky_sub", [250, 420, 0, 0, 0]
    )
    ...
    if do_irdis:
        if dit_irdis is None:
            raise ValueError(
                "dit_irdis should be provided if do_irdis is True"
            )
    else:
        dit_irdis = None
    ...
    nproc = params_calib.get(
        "nproc", int(cpu_count() / 2)
    )  # number of processors to use, default cpu_count()/2 for efficiency
    # call command esorex
    ...
    if do_irdis:
        ...
        if 1 in to_do:
            if verbose:
                print(
                    "*** 1. IRDIS: Collating darks (not used if SKY observations were made) ***",
                    flush=True,
                )
            # OBJECT
            if (
                not isfile(outpath_irdis_sof + "master_dark.sof")
                or overwrite_sof
            ):
            ...
            if (
                not isfile(outpath_irdis_fits + "master_dark.fits")
                or overwrite_sof
                or overwrite_fits
            ):
                ...
                command += (
                    " --ird.master_dark.outfilename={}master_dark.fits".format(
                        outpath_irdis_fits
                    )
                )
        ...
        if 2 in to_do:
            ...
            if len(gain_list_irdis) > 0:
                ...
                # OBJECT
                if (
                    not isfile(outpath_irdis_sof + "master_gain.sof")
                    or overwrite_sof
                ):
                    with open(
                        outpath_irdis_sof + "master_gain.sof", "w+"
                    ) as f:
                        ...
                        f.write(
                            "{}master_badpixelmap.fits".format(
                                outpath_irdis_fits
                            )
                            + "\t"
                ...
                if (
                    not isfile(outpath_irdis_fits + "master_gain.fits")
                    or overwrite_sof
                    or overwrite_fits
                ):
                    ...
                    command += (
                        " --ird.gain.nonlin_filename={}nonlin_map.fits".format(
                            outpath_irdis_fits
                        )
                    )
                ...
                if verbose:
                    print(
                        "*** 2. IRDIS: No GAIN files found, this optional step will be skipped",
                        flush=True,
                    )
        ...
        if 3 in to_do or 4 in to_do:
            if verbose:
                print(
                    "*** 3. IRDIS: Compiling SKY backgrounds ***", flush=True
                )
            # OBJ
            # sky or ins bg list?
            ...
            if len(sky_list_irdis) > 0 and good_sky_irdis is not None:
                ...
                print(
                    "Sky subtraction proxy based on median background value",
                    flush=True,
                )
            ...
            if len(psf_sky_list_irdis) > 0:
                if -1 in good_psf_sky_irdis:
                    ...
                    # just take the first (closest difference in time to that of consecutive SCIENCE cubes - reproduce best the remanence effect)
                    write_fits(
                        "{}master_sky_psf_cube.fits".format(
                            outpath_irdis_fits
                        ),
                        master_psf_sky,
                    )
        ...
        if 4 in to_do:
            if verbose:
                print(
                    "*** 4. IRDIS: Calculating master FLAT-FIELDS ***",
                    flush=True,
                )
            if (
                not isfile(outpath_irdis_sof + "master_flat.sof")
                or overwrite_sof
            ):
                ...
                with open(outpath_irdis_sof + "master_flat.sof", "w+") as f:
                    ...
                    if len(flat_dark_list_irdis) > 0:
                        if (
                            len(flat_list_irdis) % len(flat_dark_list_irdis)
                            == 0
                            and len(flat_list_irdis)
                            / len(flat_dark_list_irdis)
                            <= 3
                        ):
                            for ii in range(len(flat_dark_list_irdis)):
                                f.write(
                                    inpath
                                    + flat_dark_list_irdis[ii]
                                    + "\t"
                                    + "IRD_DARK_RAW\n"
                                )
                        else:
                            f.write(
                                "{}master_dark.fits".format(outpath_irdis_fits)
                                + "\t"
                                + "IRD_MASTER_DARK\n"
                            )
                    else:
                    ...
                    f.write(
                        "{}master_badpixelmap.fits".format(outpath_irdis_fits)
                        + "\t"
                        + "IRD_STATIC_BADPIXELMAP"
                    )

            if (
                not isfile(outpath_irdis_fits + "master_flat.fits")
                or overwrite_sof
                or overwrite_fits
            ):
        ...
        if 5 in to_do:
            ...
            if n_sci > 0:
                ...
                # SKY PCA-SUBTR
                if (
                    isfile("{}master_sky_cube.fits".format(outpath_irdis_fits))
                    and pca_subtr
                ):
                    ...
                    for ii in range(n_sci):
                        hdulist_sci = fits.open(
                            inpath + sci_list_irdis[ii],
                        ...
                        for s in range(n_s):
                            ...
                            tmp = (
                                tmp
                                - np.median(master_sci_sky_tmp, axis=0)
                                - np.median(
                            ...
                            if mask_pca_sky_sub[2] > 0:
                                mask_tmp = create_ringed_spider_mask(
                                    tmp.shape,
                                    mask_pca_sky_sub[1],
                                ...
                                star_coords_xy[ii, 1, s] = coords_tmp[0]
                                star_coords_xy[ii, 0, s] = coords_tmp[1]
                                mask_tmp_shift1 = frame_shift(
                                    mask_circ1,
                                    int(star_coords_xy[ii, 1, s] - cy_tmp),
                                    int(star_coords_xy[ii, 0, s] - cx_tmp),
                                    border_mode="constant",
                                )
                                mask_tmp_shift2 = frame_shift(
                                    mask_circ2,
                                    int(star_coords_xy[ii, 1, s] - cy_tmp),
                                    int(star_coords_xy[ii, 0, s] - cx_tmp),
                                    border_mode="constant",
                                ...
                                mask_arr[ii][s] = (
                                    mask_tmp_shift1 * mask_tmp_shift2
                                )
                            ...
                            if master_sci_sky_tmp.shape[0] == 1:
                                msg = "WARNING: master sky cube has only 1 frame. "
                                msg += (
                                    "Only direct SKY subtraction is performed."
                                )
                                print(msg, flush=True)
                            else:
                                # ACTUAL PCA SUBTRACTION
                                sci_cube_tmp = cube_subtract_sky_pca(
                                    sci_cube_tmp,
                            ...
                            sci_cube[:, :, xcuts[s] : xcuts[s + 1]] = (
                                sci_cube_tmp
                            )
                        hdulist_sci[0].data = sci_cube
                        hdulist_sci.writeto(
                            inpath + label_ss + sci_list_irdis[ii],
                            output_verify="ignore",
                            overwrite=True,
                        )
                        write_fits(
                            "{}master_masks_for_PCA_sky.fits".format(
                                outpath_irdis_fits
                            ),
                            mask_arr,
                        )
                    ...
                    hdulist.writeto(
                        "{}{}master_dark.fits".format(
                            outpath_irdis_fits, label_fd
                        ),
                        output_verify="ignore",
                        overwrite=True,
                    )
                ...
                # ELSE: we compute the master sky with sph sky recipe, which will then be passed to the reduction recipe
                    ...
                    sky_list_irdis = dico_lists["sky_list_irdis"]
                    if (
                        not isfile(outpath_irdis_sof + "master_bg.sof")
                        or overwrite_sof
                    ):
                        ins_bg_list_irdis = dico_lists["ins_bg_list_irdis"]
                        with open(
                            outpath_irdis_sof + "master_bg.sof", "w+"
                        ) as f:
                    ...
                    if len(sky_list_irdis) > 0:
                        if (
                            not isfile(outpath_irdis_fits + "sky_bg.fits")
                            or overwrite_sof
                            or overwrite_fits
                        ):
                        ...
                        if (
                            not isfile(outpath_irdis_fits + "ins_bg.fits")
                            or overwrite_sof
                            or overwrite_fits
                        ):
            ...
            if n_cen > 0:
                ...
                if (
                    ...
                    for ii in range(n_cen):
                        hdulist_cen = fits.open(
                            inpath + cen_list_irdis[ii],
                            ignore_missing_end=False,
                            memmap=True,
                        )
                        ...
                        for s in range(n_s):
                            ...
                            tmp = (
                                tmp
                                - np.median(master_cen_sky_tmp, axis=0)
                                - np.median(
                            ...
                            mask_arr[ii][s][np.where(bp_map_tmp)] = 0
                            # SUBTRACT MEDIAN SKY (incl. dark) FIRST
                            med_sky = np.median(master_cen_sky_tmp, axis=0)
                            for ss in range(cen_cube_tmp.shape[0]):
                                cen_cube_tmp[ss] -= med_sky
                            for sk in range(master_cen_sky_tmp.shape[0]):
                                master_cen_sky_tmp[sk] -= med_sky
                            if master_cen_sky_tmp.shape[0] == 1:
                                msg = "WARNING: master cen sky cube has only 1 frame. "
                                msg += (
                                    "Only direct SKY subtraction is performed."
                                )
                                print(msg, flush=True)
                                ...
                                # ACTUAL PCA SUBTRACTION
                                cen_cube_tmp = cube_subtract_sky_pca(
                                    cen_cube_tmp,
                            ...
                            cen_cube[:, :, xcuts[s] : xcuts[s + 1]] = (
                                cen_cube_tmp
                            )
                        hdulist_cen[0].data = cen_cube
                        hdulist_cen.writeto(
                            inpath + label_ss + cen_list_irdis[ii],
                            output_verify="ignore",
                            overwrite=True,
                        )
                    write_fits(
            ...
            if n_psf > 0:
                ...
                if (
                    ...
                    for ii in range(n_psf):
                        hdulist_psf = fits.open(
                            inpath + psf_list_irdis[ii],
                            ignore_missing_end=False,
                            memmap=True,
                        )
                        psf_cube = hdulist_psf[0].data
                        ...
                        for s in range(n_s):
                            ...
                            tmp = (
                                tmp
                                - np.median(master_psf_sky_tmp, axis=0)
                                - np.median(
                            ...
                            for i in range(master_psf_sky_tmp.shape[0]):
                                med_sky_lvl.append(
                                    master_psf_sky_tmp[i][
                                        np.where(mask_arr[ii][s])
                                    ]
                                )
                            med_sky_lvl = np.median(med_sky_lvl)
                            med_psf_lvl = []
                            ...
                            # PCA-sky subtraction
                            master_psf_sky_tmp = (
                                master_psf_sky_tmp - med_sky_lvl + med_psf_lvl
                            )
                            psf_cube_tmp = cube_subtract_sky_pca(
                                psf_cube_tmp,
                            ...
                            psf_cube[:, :, xcuts[s] : xcuts[s + 1]] = (
                                psf_cube_tmp
                            )
                        hdulist_psf[0].data = psf_cube
                        hdulist_psf.writeto(
                            inpath + label_ss + psf_list_irdis[ii],
                            output_verify="ignore",
                            overwrite=True,
                        )
                    write_fits(
                        "{}master_masks_for_PCA_sky_psf.fits".format(
                            outpath_irdis_fits
                        ),
                        mask_arr,
                    )
                ...
                # ELSE: we compute the master sky with sph sky recipe, which will then be passed to the reduction recipe
                    ...
                    if (
                        not isfile(outpath_irdis_sof + "master_bg_psf.sof")
                        or overwrite_sof
                    ):
                        psf_sky_list_irdis = dico_lists["psf_sky_list_irdis"]
                        psf_ins_bg_list_irdis = dico_lists[
                            "psf_ins_bg_list_irdis"
                        ]
                        with open(
                            outpath_irdis_sof + "master_bg_psf.sof", "w+"
                        ) as f:
                            if len(psf_sky_list_irdis) > 0:
                                for ii in range(len(sky_list_irdis)):
                                    f.write(
                    ...
                    if len(psf_sky_list_irdis) > 0:
                        if (
                            not isfile(outpath_irdis_fits + "psf_sky_bg.fits")
                            or overwrite_sof
                            or overwrite_fits
                        ):
                            command = "{} sph_ird_sky_bg".format(com_esorex)
                        ...
                        if (
                            ...
                            command = "{} sph_ird_ins_bg".format(com_esorex)
                            command += " --ird.ins_bg.outfilename={}psf_ins_bg.fits".format(
                                outpath_irdis_fits
                            )
        ...
        if 6 in to_do:
            ...
            if len(sci_list_irdis) > 0:
                for ii, file in enumerate(sci_list_irdis):
                    if (
                        not isfile(
                            outpath_irdis_sof + "OBJECT{:.0f}.sof".format(ii)
                        )
                        or overwrite_sof
                    ):
                        with open(
                            ...
                            f.write(
                                ...
                                + sci_list_irdis[ii]
                                + "\t"
                                + f"IRD_SCIENCE_{label_method}_RAW\n"
                            )
                            if pca_subtr:
                                f.write(
                                    "{}{}master_dark.fits".format(
                                ...
                                if isfile(
                                    "{}sky_bg.fits".format(outpath_irdis_fits)
                                ):
                                    f.write(
                                        "{}sky_bg.fits".format(
                                            outpath_irdis_fits
                                    ...
                                    "{}ins_bg.fits".format(outpath_irdis_fits)
                                ):
                                    f.write(
                                        "{}ins_bg.fits".format(
                                            outpath_irdis_fits
                            ...
                            f.write(
                                "{}master_flat.fits".format(outpath_irdis_fits)
                                + "\t"
                                + "IRD_FLAT_FIELD\n"
                            )
                            if isfile(f1):
                                f.write(f1 + "\t" + "IRD_FILTER_TABLE\n")
                            f.write(
                                "{}FINAL_badpixelmap.fits".format(
                                    outpath_irdis_fits
                                )
                                + "\t"
                                + "IRD_STATIC_BADPIXELMAP"
                    ...
                    if (
                        not isfile(outpath_irdis_fits + f"{file}_left.fits")
                        or not isfile(
                            outpath_irdis_fits + f"{file}_right.fits"
                        )
                        or overwrite_sof
                        or overwrite_fits
                    ):
                        _reduce_irdis_esorex(
                            outpath_irdis_fits,
                            outpath_irdis_sof,
                            file,
                            ii,
            ...
            if len(cen_list_irdis) > 0:
                for ii, file in enumerate(cen_list_irdis):
                    if (
                        not isfile(
                            outpath_irdis_sof + "CEN{:.0f}.sof".format(ii)
                        )
                        or overwrite_sof
                    ):
                        with open(
                            outpath_irdis_sof + "CEN{:.0f}.sof".format(ii), "w"
                        ) as f:
                            f.write(
                                inpath
                                + label_ss
            ...
            if len(psf_list_irdis) > 0:
                for ii, file in enumerate(psf_list_irdis):
                    if (
                        not isfile(
                            outpath_irdis_sof + "PSF{:.0f}.sof".format(ii)
                        )
                        or overwrite_sof
                    ):
                        with open(
                            outpath_irdis_sof + "PSF{:.0f}.sof".format(ii), "w"
                        ) as f:
                            f.write(
                                inpath
                                + label_ss
            ...
            if not pca_subtr or not pca_subtr_psf:
                lab_ird = [
                    "_left",
                    "_right",
                ]  # always for IRDIS; whether CI or DBI
                file_list = os.listdir(outpath_irdis_fits)
                for lr in lab_ird:
                    products_list = [
                        x
                        for x in file_list
                        if (x.startswith(instr) and x.endswith(f"{lr}.fits"))
                    ]
                    for pp, prod in enumerate(products_list):
                        tmp, head_tmp = open_fits(
                            outpath_irdis_fits + prod,
                            header=True,
                            verbose=False,
                        )
                        if head_tmp[
                            ...
                            not manual_sky_irdis and pca_subtr
                        ):
                            continue
                        elif head_tmp[
                            "HIERARCH ESO DET SEQ1 DIT"
                        ...
                        peak_y, peak_x = peak_coordinates(
                            tmp_med,
                            fwhm=4,
                            approx_peak=None,
                            search_box=None,
                        ...
                        for zz in range(tmp.shape[0]):
                            ...
                            if edge < rad / 4:
                                ...
                                aper = CircularAperture(
                                    ap, r=int(rad / 8)
                                )  # small r because risk of aberrant values near edges
                            ...
                            # else (ie. if star near center, estimate bkg flux in an annulus)
                                ...
                                ann_aper = CircularAnnulus(
                                    (peak_x, peak_y), r_in=r_in, r_out=r_out
                                )
    ...
    if do_ifs:
        ...
        if 10 in to_do or 15 in to_do or 17 in to_do:
            ...
            with open(outpath_ifs_sof + "master_dark.sof", "w+") as f:
                for ii in range(len(dark_list_ifs)):
                    dark_cube, dark_head = open_fits(
                        inpath + dark_list_ifs[ii], header=True, verbose=False
                    )
                    ...
                    f.write(
                        inpath + dark_list_ifs[ii] + "\t" + "IFS_DARK_RAW\n"
                    )
                    if dark_cube.ndim == 3:
                        if ii == 0:
                            master_dark_cube = [
                                dark_cube[i] for i in range(dark_cube.shape[0])
                            ]
                        else:
                            tmp = [
                                dark_cube[i] for i in range(dark_cube.shape[0])
            ...
            write_fits(
                outpath_ifs_fits + "master_dark_cube.fits",
                master_dark_cube,
                verbose=False,
            )  # saves cube of all darks
            if (
                not isfile(outpath_ifs_fits + "master_dark.fits")
                or overwrite_sof
                or overwrite_fits
            ):
                ...
                command += (
                    " --ifs.master_dark.outfilename={}master_dark.fits".format(
                        outpath_ifs_fits
                    )
                )
                command += " --ifs.master_dark.badpixfilename={}master_badpixelmap.fits".format(
                    outpath_ifs_fits
                )
            ...
            if indiv_fdark and len(dico_lists["dit_ifs_flat"]) > 1:
                ...
                if verbose:
                    print(
                        "Flat DITs: {} sec, attempting to match with darks of same DIT".format(
                            dit_ifs_flat_list
                        ),
                        flush=True,
                    )
                for nn, fdit in enumerate(dit_ifs_flat_list):
                    dark_cube = open_fits(
                        inpath + fdark_list_ifs[0], verbose=False
                    ...
                    with open(
                        outpath_ifs_sof + "master_fdark{:.0f}.sof".format(nn),
                        "w+",
                    ) as f:
                        for dd in range(nfd):
                            # loop over darks and grab the ones that match the exposure time of the flat
                            dark_cube, fdark_head = open_fits(
                                inpath + fdark_list_ifs[dd],
                                header=True,
                                verbose=False,
                            )
                            ...
                            if dit_fdark == fdit:
                                ...
                                master_dark_cube[
                                    counter : counter + dark_cube.shape[0]
                                ] = np.copy(dark_cube)
                                counter += dark_cube.shape[0]
                    write_fits(
                        outpath_ifs_fits
                        + "master_dark_cube{:.0f}.fits".format(nn),
                    ...
                    if (
                        ...
                        command += " {}master_fdark{:.0f}.sof".format(
                            outpath_ifs_sof, nn
                        )
                        os.system(command)
                    ...
                    for nf in range(len(all_flat_lists_ifs)):
                        for ff, ff_name in enumerate(all_flat_lists_ifs[nf]):
                            ...
                            if dit_flat == fdit:
                                ...
                                hdulist.writeto(
                                    inpath + label_ds + ff_name,
                                    output_verify="ignore",
                                    overwrite=True,
                                )

                np.save(
                    outpath_ifs_fits + "flat_dark_dits.npy", fdit_list_nn
                )  # used later in spec_pos
                # make vmin, vmax and labels with integration time for plot
                labels = tuple(
                    [
                        s + " sec"
                        for s in [
                            str(dit_ifs_flat_list)
                            for dit_ifs_flat_list in dit_ifs_flat_list
                        ]
                    ]
                )
                vmax = tuple(
                    np.percentile(frame, q=98) for frame in all_median_darks
                )
                vmin = tuple(
                    np.percentile(frame, q=2) for frame in all_median_darks
                )
                plot_frames(
                    tuple(all_median_darks),
                    vmax=vmax,
                    vmin=vmin,
                    cmap="inferno",
                    dpi=300,
            ...
            if indiv_fdark:
                ...
                os.system(
                    "rsync -va {}master_badpixelmap{:.0f}.fits {}master_badpixelmap.fits".format(
                        outpath_ifs_fits,
                ...
                hdulist = fits.open(
                    "{}master_dark{}.fits".format(
                        outpath_ifs_fits,
                        "{:.0f}".format(len(dit_ifs_flat_list) - 1),
                    ),
                    ignore_missing_end=False,
                    memmap=True,
                )  # open longest DIT dark
                dark = hdulist[0].data
                hdulist[0].data = np.zeros_like(dark)
                hdulist.writeto(
                    "{}{}master_dark.fits".format(outpath_ifs_fits, label_fd),
                    output_verify="ignore",
                    overwrite=True,
                )
        ...
        if 11 in to_do:
            ...
            if len(gain_list_ifs) > 0:
                ...
                if (
                    not isfile(outpath_ifs_sof + "master_gain.sof")
                    or overwrite_sof
                ):
                ...
                if (
                    not isfile(outpath_ifs_fits + "master_gain_map.fits")
                    or overwrite_sof
                    or overwrite_fits
                ):
                    ...
                    command += (
                        " --ifs.gain.nonlin_filename={}nonlin_map.fits".format(
                            outpath_ifs_fits
                        )
                    )
                    command += " --ifs.gain.nonlin_bpixname={}nonlin_badpixelmap.fits".format(
                        outpath_ifs_fits
                    )
                ...
                if (
                    not isfile(outpath_ifs_fits + "master_gain_map.pdf")
                    or overwrite_fits
                ):
                    gain = open_fits(
                        outpath_ifs_fits + "master_gain_map.fits",
                        verbose=False,
                    )
                    plot_frames(
                        gain,
                        vmax=np.percentile(gain, q=99.5),
                        vmin=np.percentile(gain, q=0.5),
                        cmap="inferno",
                        dpi=300,
                        label="master_gain_map.fits",
                        save=outpath_ifs_fits + "master_gain_map.pdf",
                    )

            elif len(gain_list_ifs) == 0:
                if verbose:
                    print(
                        "*** 11. IFS: No GAIN files found, this optional step will be skipped",
                        flush=True,
                    )
        ...
        if 12 in to_do:
            if verbose:
                print(
                    "*** 12. IFS: Calculating detector FLAT-FIELD ***",
                    flush=True,
                )
            flat_list_ifs_det_BB = dico_lists["flat_list_ifs_det_BB"]
            flat_list_ifs_det = dico_lists["flat_list_ifs_det"]
            ...
            if xtalk_corr:
                for ii in range(len(flat_list_ifs_det_BB)):
                    hdul = fits.open(
                        inpath + label_ds + flat_list_ifs_det_BB[ii]
                    )
                    cube = hdul[0].data
                    # CROSS-TALK CORR
                    for j in range(cube.shape[0]):
                        cube[j] = sph_ifs_correct_spectral_xtalk(
                            cube[j], boundary="fill", fill_value=0
                        )
                    hdul[0].data = cube
            ...
            with open(outpath_ifs_sof + "preamp.sof", "w+") as f:
                ...
                if ("FLAT" in dark_ifs and not indiv_fdark) or indiv_fdark:
                    f.write(
                        "{}master_dark.fits".format(
                            outpath_ifs_fits + label_fd
                        )
                        + "\t"
                        + "IFS_MASTER_DARK\n"
                    )
                f.write(
                    "{}master_badpixelmap.fits".format(outpath_ifs_fits)
                    + "\t"
                    + "IFS_STATIC_BADPIXELMAP"
                )

            if (
                not isfile(outpath_ifs_fits + "preamp_l5.fits")
                or overwrite_sof
                or overwrite_fits
            ):
                command = "{} sph_ifs_master_detector_flat".format(com_esorex)
                command += (
                    " --ifs.master_detector_flat.badpix_lowtolerance=0.2"
                )
            ...
            if large_scale_flat:
                ...
                for kk in range(1, 5):
                    with open(
                        outpath_ifs_sof
                        + "large_scale_flat_l{:.0f}.sof".format(kk),
                        "w+",
                    ) as f:
                        run_rec = False
                        counter = 0
                        for ii in range(len(flat_list_ifs_det)):
                            header = open_header(
                                inpath + flat_list_ifs_det[ii]
                            )
                            if (
                                "HL{:.0f}".format(kk)
                                in header["HIERARCH ESO INS2 CAL"]
                            ):
                                f.write(
                                    inpath
                                    + lab_flat
                        ...
                        if (
                            "FLAT" in dark_ifs and not indiv_fdark
                        ) or indiv_fdark:
                            f.write(
                                "{}master_dark.fits".format(
                                    outpath_ifs_fits + label_fd
                        ...
                        f.write(
                            "{}preamp_l5.fits".format(outpath_ifs_fits)
                            + "\t"
                            + "IFS_PREAMP_FLAT\n"
                        )
                    ...
                    if run_rec and (
                        not isfile(
                            outpath_ifs_fits
                            + "large_scale_flat_l{:.0f}.fits".format(kk)
                        )
                        or overwrite_sof
                        or overwrite_fits
                    ):
                        command = "{} sph_ifs_master_detector_flat".format(
                            com_esorex
                        )
                        command += " --ifs.master_detector_flat.badpix_lowtolerance=0.2"
                        command += (
                            " --ifs.master_detector_flat.badpix_uptolerance=5."
                        )
                        command += (
                        ...
                        if flat_fit and counter > 4:
                            command += (
                                " --ifs.master_detector_flat.robust_fit=TRUE"
                            )
                        command += " --ifs.master_detector_flat.outfilename={}tmp1.fits".format(
                            outpath_ifs_fits
                        )
                ...
                if (
                    not isfile(outpath_ifs_sof + "large_scale_flat_BB.sof")
                    or overwrite_sof
                ):
                    flat_list_ifs_det_BB = dico_lists["flat_list_ifs_det_BB"]
                    with open(
                        outpath_ifs_sof + "large_scale_flat_BB.sof", "w+"
                    ) as f:
                        for ii in range(len(flat_list_ifs_det_BB)):
                            f.write(
                                inpath
                ...
                if (
                    not isfile(outpath_ifs_fits + "large_scale_flat_l5.fits")
                    or overwrite_sof
                    or overwrite_fits
                ):
                    command = "{} sph_ifs_master_detector_flat".format(
                    ...
                    command += (
                        " --ifs.master_detector_flat.smoothing_length=5."
                    )
            ...
            if large_scale_flat != "all":
                ...
                for kk in range(1, 5):
                    with open(
                        outpath_ifs_sof
                        + "master_flat_det_l{:.0f}.sof".format(kk),
                        "w+",
                    ) as f:
                        run_rec = False
                        counter = 0
                        ...
                        if large_scale_flat:
                            f.write(
                                "{}large_scale_flat_l{:.0f}.fits".format(
                                    outpath_ifs_fits, kk
                                )
                                + "\t"
                    ...
                    if run_rec and (
                        not isfile(
                            outpath_ifs_fits
                            + "master_flat_det_l{:.0f}.fits".format(kk)
                        )
                        or overwrite_sof
                        or overwrite_fits
                    ):
                ...
                if (
                    not isfile(outpath_ifs_sof + "master_flat_det_BB.sof")
                    or overwrite_sof
                ):
                    flat_list_ifs_det_BB = dico_lists["flat_list_ifs_det_BB"]
                    with open(
                        outpath_ifs_sof + "master_flat_det_BB.sof", "w+"
                    ) as f:
                        for ii in range(len(flat_list_ifs_det_BB)):
                            f.write(
                                inpath
                        ...
                        if large_scale_flat:
                            f.write(
                                "{}large_scale_flat_l5.fits".format(
                                    outpath_ifs_fits
                                )
                                + "\t"
                                + "IFS_LARGE_SCALE_FLAT\n"
                ...
                if (
                    not isfile(outpath_ifs_fits + "master_flat_det_l5.fits")
                    or overwrite_sof
                    or overwrite_fits
                ):
                    command = "{} sph_ifs_master_detector_flat".format(
            ...
            flat_fits = [
                open_fits(
                    outpath_ifs_fits
                    + "master_flat_det_l{:.0f}.fits".format(f),
                    verbose=False,
                )
                for f in range(1, 4)
            ]
            if isfile(outpath_ifs_fits + "master_flat_det_l4.fits"):
                flat_fits.append(
                    open_fits(
                        outpath_ifs_fits + "master_flat_det_l4.fits",
                        verbose=False,
                    )
                )
            flat_fits.append(
                open_fits(
                    outpath_ifs_fits + "master_flat_det_l5.fits", verbose=False
                )
            )
            # label for each plot saying laser number
            labels = [
                "Laser " + s
                for s in [str(f) for f in range(1, len(flat_fits))]
            ]
            labels.append("BB")
            ...
            plot_frames(
                ...
                cmap="inferno",
                dpi=300,
                save=f"{outpath_ifs_fits}master_flat_det_l1-5.pdf",
            )
        ...
        if 13 in to_do:
            if verbose:
                print(
                    "*** 13. IFS: Calculating spectra positions ***",
                    flush=True,
                )
            dit_ifs_flat_list = dico_lists["dit_ifs_flat"]
            nfdits = len(dit_ifs_flat_list)
            if (
                not isfile(outpath_ifs_sof + "spectra_pos.sof")
                or overwrite_sof
            ):
                ...
                with open(outpath_ifs_sof + "spectra_pos.sof", "w+") as f:
                    ...
                    if large_scale_flat == "all":
                        ...
                        f.write(
                            "{}master_flat_det_l5.fits".format(
                                outpath_ifs_fits
                            )
                            + "\t"
                            + "IFS_INSTRUMENT_FLAT_FIELD\n"
                    ...
                    if indiv_fdark:
                        ...
                        fdit_list_nn = np.load(
                            outpath_ifs_fits + "flat_dark_dits.npy"
                        )
                        for ff in range(nfdits):
                            if fdit_list_nn[ff][1] == header["EXPTIME"]:
                                ...
                                print(
                                    "no master dark with appropriate DIT was found for spec_pos",
                                    flush=True,
                                )
                                set_trace()
                        f.write(
                        ...
                        f.write(
                            "{}master_dark.fits".format(outpath_ifs_fits)
                            + "\t"
                            + "IFS_MASTER_DARK\n"
                        )
                    if mode == "YJ":
                        f.write(f2 + "\t" + "IFS_LENSLET_MODEL\n")

            if (
                not isfile(outpath_ifs_fits + "spectra_pos.fits")
                or overwrite_sof
                or overwrite_fits
            ):
        ...
        if 14 in to_do:
            if verbose:
                print(
                    "*** 14. IFS: Calculating total instrument flat-field ***",
                    flush=True,
                )
            if (
                not isfile(outpath_ifs_fits + "master_flat_tot.fits")
                or overwrite_sof
                or overwrite_fits
            ):
                if (
                    not isfile(outpath_ifs_sof + "master_flat_tot.sof")
                    or overwrite_sof
                ):
                    flat_list_ifs = dico_lists["flat_list_ifs"]
                    with open(
                        outpath_ifs_sof + "master_flat_tot.sof", "w+"
                    ) as f:
                        ...
                        f.write(
                            "{}spectra_pos.fits".format(outpath_ifs_fits)
                            + "\t"
                            + "IFS_SPECPOS\n"
                        )
                        for jj in range(1, 6):
                            if not large_scale_flat or (
                                jj == 5 and large_scale_flat == "some"
                            ):
                                lab_sof = (
                                    "{}master_flat_det_l{:.0f}.fits".format(
                                        outpath_ifs_fits, jj
                                    )
                                )  # v5c
                            else:
                ...
                if (
                    ...
                    if len(flat_list_ifs) > 1 and flat_fit:
                        ...
                        command += (
                            " --ifs.instrument_flat.badpix_lowtolerance=0.2"
                        )
                        command += (
                            " --ifs.instrument_flat.badpix_uptolerance=5.0"
                        )
                    ...
                    if illum_pattern_corr:
                        command += (
                            " --ifs.instrument_flat.use_illumination=TRUE"
                        )
        ...
        if 15 in to_do:
            ...
            if not isfile(outpath_ifs_sof + "wave_calib.sof") or overwrite_sof:
                ...
                for ii in range(len(wave_calib_list_ifs)):
                    ...
                    for nn, fdit in enumerate(dit_ifs_flat_list):
                        ## subtract with PCA
                        master_dark_cube = open_fits(
                            outpath_ifs_fits + f"master_dark_cube{nn}.fits",
                            verbose=False,
                        )
                        if fdit == dit_wc:
                    ...
                    hdul.writeto(
                        inpath + skysub_lab_IFS + wave_calib_list_ifs[ii],
                        output_verify="ignore",
                        overwrite=True,
                    )

                    cube = cube_fix_badpix_clump(
                        cube,
                        bpm_mask=bpmap,
                        correct_only=True,
                        cy=None,
                    ...
                    if not isdir(inpath + lab_wc):
                        os.makedirs(inpath + lab_wc)
                    hdul.writeto(
                        inpath + bpcorr_lab_IFS + wave_calib_list_ifs[ii],
                        output_verify="ignore",
                        overwrite=True,
                    )
                ...
                with open(outpath_ifs_sof + "wave_calib.sof", "w+") as f:
                    ...
                    f.write(
                        "{}{}.fits".format(outpath_ifs_fits, master_flatname)
                        + "\t"
                        + "IFS_INSTRUMENT_FLAT_FIELD\n"
                    )

            if (
                not isfile(outpath_ifs_fits + "wave_calib.fits")
                or overwrite_sof
                or overwrite_fits
            ):
                ...
                if wc_win_sz != 4:
                    command += (
                        " --ifs.wave_calib.fit_window_size={:.0f}".format(
                            wc_win_sz
                        )
                    )
                if poly_order_wc != 2:
                    command += " --ifs.wave_calib.polyfit_order={:.0f}".format(
                        poly_order_wc
                    )
                command += (
                    " --ifs.wave_calib.outfilename={}wave_calib.fits".format(
                        outpath_ifs_fits
                    )
                )
        ...
        if 16 in to_do:
            if verbose:
                print(
                    "*** 16. IFS: Calibrating final IFU flat-fields ***",
                    flush=True,
                )
            if (
                not isfile(outpath_ifs_fits + "master_flat_ifu.fits")
                or overwrite_sof
                or overwrite_fits
            ):
                if (
                    not isfile(outpath_ifs_sof + "master_flat_ifu.sof")
                    or overwrite_sof
                ):
                    # flat_list_ifs = dico_lists['flat_list_ifs_det'] # v1
                    flat_list_ifs = dico_lists[
                        "flat_list_ifs"
                    ]  # v2 (as manual?)
                    # flat_list_ifs_BB = dico_lists['flat_list_ifs_det_BB']
                    with open(
                        outpath_ifs_sof + "master_flat_ifu.sof", "w+"
                    ) as f:
                        for ii in range(len(flat_list_ifs)):
                            f.write(
                                inpath
                        ...
                        f.write(
                            "{}wave_calib.fits".format(outpath_ifs_fits)
                            + "\t"
                            + "IFS_WAVECALIB\n"
                        )
                ...
                if (
                    ...
                    command += " --ifs.instrument_flat.ifu_filename={}master_flat_ifu.fits".format(
                        outpath_ifs_fits
                    )
        ...
        if 17 in to_do and sky:
            ...
            if -1 in good_sky_list:
                ...
                # just take the first (closest difference in time to that of consecutive SCIENCE cubes - reproduce best the remanence effect)
                write_fits(
                    "{}master_sky.fits".format(outpath_ifs_fits), tmp[0]
                )
                ...
                master_sky = np.median(master_sky, axis=0)
                write_fits(
                    "{}master_sky.fits".format(outpath_ifs_fits), master_sky
                )
            else:
                counter = 0
            ...
            if len(psf_sky_list_ifs) < 1:
                ...
                write_fits(
                    "{}dit_psf_sky.fits".format(outpath_ifs_fits),
                    np.array([dit_ifs]),
                )
            else:
                write_fits(
                    "{}dit_psf_sky.fits".format(outpath_ifs_fits),
                    np.array([dit_psf_ifs]),
                )
                if -1 in good_psf_sky_list:
                    tmp = open_fits(inpath + sky_list_ifs[0])
                    # just take the first (closest difference in time to that of consecutive SCIENCE cubes - reproduce best the remanence effect)
                    write_fits(
                        "{}master_psf_sky.fits".format(outpath_ifs_fits),
                        tmp[0],
                    )
        ...
        def manual_sky_subtract(
            lab_bp: str,
            instr: str,
            corner_coords: list,
            msky_ap: int,
            outpath_ifs_fits: str,
            filetype: str,
        ) -> None:
            """
            ...
            products_list = [
                x
                for x in file_list
                if (x.startswith(lab_bp + instr) and x.endswith(".fits"))
            ]
            for pp, prod in enumerate(products_list):
                hdul = fits.open(prod)
                tmp = hdul[0].data
                for zz in range(tmp.shape[0]):
                    ...
                    for aa, ap in enumerate(corner_coords):
                        ...
                        flux_tmp = aperture_photometry(
                            tmp[zz], aper, method="exact"
                        )
                ...
                if pp == 0:
                    ...
                    plot_frames(
                        ...
                        cmap="inferno",
                        dpi=300,
                        circle=tuple(corner_coords),
                        circle_radius=msky_ap,
                        log=True,
                        vmin=0,
                        vmax=np.percentile(im, q=99.5),
                        save=outpath_ifs_fits + f"Manual_sky_{filetype}.pdf",
                    )

        # REDUCE OBJECT
        lab_distort = (
            ""  # no distortion correction possible for ifs in pipeline
        )
        if 18 in to_do:
            if verbose:
                print(
                    "*** 18. IFS: Reduce all OBJECT datacubes ***", flush=True
                )
            ...
            for ii in range(len(sci_list_ifs)):
                ...
                if sky:
                    tmp_tmp = open_fits(
                        "{}master_sky.fits".format(outpath_ifs_fits)
                    )
                    ...
                    hdul[0].data = cube
                    hdul.writeto(
                        inpath + skysub_lab_IFS + lab_bp + sci_list_ifs[ii],
                        output_verify="ignore",
                        overwrite=True,
                    )
                else:
                ...
                hdul.writeto(
                    inpath + bpcorr_lab_IFS + lab_bp + sci_list_ifs[ii],
                    output_verify="ignore",
                    overwrite=True,
                )
                ...
                if xtalk_corr:  # too aggressive
                    ...
                    hdul.writeto(
                        inpath + xtalkcorr_lab_IFS + lab_bp + sci_list_ifs[ii],
                        output_verify="ignore",
                        overwrite=True,
                    )
                    # pdb.set_trace()
                ...
                if (
                    not isfile(
                        outpath_ifs_sof
                        + "OBJECT{}{:.0f}.sof".format(lab_distort, ii)
                    )
                    or overwrite_sof
                ):
                    with open(
                        ...
                        f.write(
                            "{}master_flat_ifu.fits".format(outpath_ifs_fits)
                            + "\t"
                            + "IFS_IFU_FLAT_FIELD\n"
                        )
                        #                        if cal_bkg:
                ...
                if (
                    not isfile(
                        outpath_ifs_fits
                        + "ifs{}_{:.0f}.fits".format(lab_distort, ii)
                    )
                    or overwrite_sof
                    or overwrite_fits
                ):
                ...
                os.system(
                    "mv {} {}.".format(
                        lab_bp + instr + "*.fits", outpath_ifs_calib
                    )
                )
                os.system("rm {}{}".format(outpath_ifs_fits, "tmp*.fits"))
            write_fits(
                outpath_ifs_fits + "true_ndit_obj.fits",
                true_ndit,
                verbose=False,
            )
        ...
        if 19 in to_do:
            ...
            for ii in range(len(cen_list_ifs)):
                ...
                if sky:
                    ...
                    hdul[0].data = cube
                    hdul.writeto(
                        inpath + skysub_lab_IFS + lab_bp + cen_list_ifs[ii],
                        output_verify="ignore",
                        overwrite=True,
                    )
                else:
                ...
                if not isdir(inpath + lab_sci):
                    os.makedirs(inpath + lab_sci)
                hdul.writeto(
                    inpath + bpcorr_lab_IFS + lab_bp + cen_list_ifs[ii],
                    output_verify="ignore",
                    overwrite=True,
                )
                ...
                if xtalk_corr:  # too aggressive
                    ...
                    lab_sci = xtalkcorr_lab_IFS
                    hdul.writeto(
                        inpath + xtalkcorr_lab_IFS + lab_bp + cen_list_ifs[ii],
                        output_verify="ignore",
                        overwrite=True,
                    )
                    # pdb.set_trace()
                ...
                if (
                    not isfile(
                        outpath_ifs_sof
                        + "CEN{}{:.0f}.sof".format(lab_distort, ii)
                    )
                    or overwrite_sof
                ):
                    with open(
                ...
                if (
                    not isfile(
                        outpath_ifs_fits
                        + "ifs_cen{}_{:.0f}.fits".format(lab_distort, ii)
                    )
                    or overwrite_sof
                    or overwrite_fits
                ):
                ...
                os.system("rm {}{}".format(outpath_ifs_fits, "tmp*.fits"))
            write_fits(
                outpath_ifs_fits + "true_ndit_cen.fits",
                true_ndit,
                verbose=False,
            )
        ...
        if 20 in to_do:
            ...
            for ii in range(len(psf_list_ifs)):
                ...
                if sky:
                    ...
                    hdul[0].data = cube
                    hdul.writeto(
                        inpath + skysub_lab_IFS + lab_bp + psf_list_ifs[ii],
                        output_verify="ignore",
                        overwrite=True,
                    )
                else:
                ...
                if not isdir(inpath + lab_sci):
                    os.makedirs(inpath + lab_sci)
                hdul.writeto(
                    inpath + bpcorr_lab_IFS + lab_bp + psf_list_ifs[ii],
                    output_verify="ignore",
                    overwrite=True,
                )
                ...
                if xtalk_corr:  # too agressive
                    ...
                    lab_sci = xtalkcorr_lab_IFS
                    hdul.writeto(
                        inpath + xtalkcorr_lab_IFS + lab_bp + psf_list_ifs[ii],
                        output_verify="ignore",
                        overwrite=True,
                    )
                ...
                if (
                    not isfile(
                        outpath_ifs_sof
                        + "PSF{}{:.0f}.sof".format(lab_distort, ii)
                    )
                    or overwrite_sof
                ):
                    with open(
                ...
                if (
                    not isfile(
                        outpath_ifs_fits
                        + "ifs_psf{}_{:.0f}.fits".format(lab_distort, ii)
                    )
                    or overwrite_sof
                    or overwrite_fits
                ):
