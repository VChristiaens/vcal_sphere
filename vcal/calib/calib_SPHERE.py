...
def calib(params_calib_name='VCAL_params_calib.json') -> None:
    ...
    *******
    ...
    corner_coords = params_calib.get('corner_coords',[[39,215],[72,62],[247,78],[216,230]]) # x,y coords where the sky will be manually estimated # !!! CAREFUL IF STAR NOT CENTERED => adapt
    ...
    large_scale_flat = params_calib.get('large_scale_flat', 0) # choice between 'all' (v1.38 manual), 'some' (v1.40? cf. dr recipe), False (not used, v1.40? cf. flat recipe)
        ...
        if 10 in to_do or 15 in to_do or 17 in to_do:
            ...
            with open(outpath_ifs_sof+"master_dark.sof", 'w+') as f:
                for ii in range(len(dark_list_ifs)):
                    ...
                    if dark_head['HIERARCH ESO DET SEQ1 DIT'].round(2) == 1.65:
